...
class TestWorkflowEngine:
    ...
    def test_get_next_stages_initial(self):
        ...
        current_steps = {
            WorkflowStage.INGESTION: WorkflowStep(
                stage=WorkflowStage.INGESTION, status=WorkflowStatus.COMPLETED
            )
        }
        ...
        assert (
            WorkflowStage.LAYOUT_ANALYSIS not in next_stages
    ...
    def test_get_next_stages_multiple_dependencies(self):
        ...
        current_steps = {
            ...
            WorkflowStage.PREPROCESSING: WorkflowStep(
                stage=WorkflowStage.PREPROCESSING, status=WorkflowStatus.COMPLETED
            ),
            WorkflowStage.LAYOUT_ANALYSIS: WorkflowStep(
                stage=WorkflowStage.LAYOUT_ANALYSIS, status=WorkflowStatus.COMPLETED
            ),
            WorkflowStage.OCR: WorkflowStep(
                stage=WorkflowStage.OCR, status=WorkflowStatus.COMPLETED
            ),
        }
    ...
    def test_is_workflow_complete(self):
        ...
        complete_steps = {
            WorkflowStage.COMPLETED: WorkflowStep(
                stage=WorkflowStage.COMPLETED, status=WorkflowStatus.COMPLETED
            )
        }

        incomplete_steps = {
            WorkflowStage.EVALUATION: WorkflowStep(
                stage=WorkflowStage.EVALUATION, status=WorkflowStatus.COMPLETED
            )
        }
    ...
    def test_should_quarantine_low_accuracy(self):
        ...
        assert not self.engine.should_quarantine(
            steps, accuracy=0.9995
        )  # Above threshold
    ...
    def test_should_quarantine_critical_failure(self):
        ...
        failed_layout_steps = {
            WorkflowStage.LAYOUT_ANALYSIS: WorkflowStep(
                stage=WorkflowStage.LAYOUT_ANALYSIS, status=WorkflowStatus.FAILED
            )
        }

        failed_ocr_steps = {
            WorkflowStage.OCR: WorkflowStep(
                stage=WorkflowStage.OCR, status=WorkflowStatus.FAILED
            )
        }
        ...
        for dep in dependencies:
            if dep in self.engine.STAGE_DEPENDENCIES:
                self._check_circular_dependency(
                    dep, self.engine.STAGE_DEPENDENCIES[dep], visited.copy()
                )
