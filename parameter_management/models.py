...
class Parameter(Base):
    ...
    )  # e.g., "accuracy", "extraction", "ui"
    subcategory = Column(
        String(100), index=True
    )  # e.g., "drift_detection", "title_extraction"
    ...
    versions = relationship(
        "ParameterVersion", back_populates="parameter", cascade="all, delete-orphan"
    )
    overrides = relationship(
        "ParameterOverride", back_populates="parameter", cascade="all, delete-orphan"
    )
...
class ParameterVersion(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    parameter_id = Column(
        UUID(as_uuid=True), ForeignKey("parameters.id"), nullable=False
    )
    created_at = Column(
        DateTime(timezone=True),
        default=lambda: datetime.now(timezone.utc),
    ...
    status = Column(
        Enum(ParameterStatus), nullable=False, default=ParameterStatus.DRAFT, index=True
    )
    ...
    )  # SHA-256 hash of value for deduplication
    ...
    )  # manual, automated, migration, rollback
    ...
    rolled_back_from_version = Column(
        UUID(as_uuid=True), ForeignKey("parameter_versions.id")
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "parameter_id", "version_number", name="uq_parameter_version_number"
        ),
    )
...
class ParameterOverride(Base):
    ...
    __table_args__ = (
        Index(
            "idx_parameter_overrides_param_scope",
            "parameter_id",
            "scope",
            "scope_identifier",
        ),
...
class ParameterAuditLog(Base):
    ...
    )  # create, update, delete, activate, deactivate, override, rollback
    object_type = Column(
        String(50), nullable=False
    )  # parameter, version, override, change_request
...
class ParameterTemplate(Base):
    ...
    )  # JSON schema defining the template
...
class ParameterSnapshot(Base):
    ...
    snapshot_type = Column(
        String(50), default="manual"
    )  # manual, scheduled, pre_deployment, pre_change
    ...
    related_change_request_id = Column(
        UUID(as_uuid=True), ForeignKey("parameter_change_requests.id")
    )
...
def get_active_parameter_value(
    ...
    parameter = (
        ...
        .options(joinedload(Parameter.versions), joinedload(Parameter.overrides))
    ...
    applicable_overrides = overrides_query.order_by(
        ParameterOverride.priority.desc()
    ).all()
...
def get_parameter_history(
    session: Session, parameter_key: str, limit: int = 50
