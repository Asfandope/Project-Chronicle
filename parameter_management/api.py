...
class ParameterCreateRequest(BaseModel):
    ...
    data_type: str = Field(
        ..., pattern=r"^(string|integer|float|boolean|json|array|object)$"
    )
    default_value: Any
...
class RollbackRequestAPI(BaseModel):
    ...
    def at_least_one_target(cls, v, values):
        if (
            not v
            and not values.get("target_version_id")
            and not values.get("target_snapshot_id")
        ):
            raise ValueError(
                "Either target_version_id or target_snapshot_id must be provided"
            )
        return v
...
class SnapshotCreateRequest(BaseModel):
    ...
    snapshot_type: str = Field(
        "manual", pattern=r"^(manual|scheduled|pre_deployment|pre_change)$"
    )
...
def create_parameter_management_api(
    db_session_factory, prefix: str = "/api/v1/parameters"
) -> FastAPI:
    ...
    async def list_parameters(
        ...
        parameters = (
            query.order_by(Parameter.category, Parameter.key)
            .offset(offset)
            .limit(page_size)
            .all()
        )
    ...
    async def get_parameter(
        ...
        if not parameter:
            raise HTTPException(
                status_code=404, detail=f"Parameter '{parameter_key}' not found"
            )

        try:
            current_value = parameter_service.get_parameter_value(
                db, parameter_key, brand
            )
    ...
    @app.put(
        f"{prefix}/{{parameter_key}}/value", response_model=ParameterVersionResponse
    )
    async def update_parameter_value(
        ...
        try:
            ...
            if not version:
                raise HTTPException(
                    status_code=202, detail="Change request created - approval required"
                )
    ...
    @app.get(
        f"{prefix}/{{parameter_key}}/versions",
    ...
    @app.post(
        f"{prefix}/{{parameter_key}}/overrides",
        response_model=ParameterOverrideResponse,
    )
    ...
    @app.get(
        f"{prefix}/{{parameter_key}}/overrides",
        response_model=List[ParameterOverrideResponse],
    )
    async def get_parameter_overrides(
        ...
        query = db.query(ParameterOverride).filter(
        ...
        overrides = query.order_by(
            desc(ParameterOverride.priority), desc(ParameterOverride.created_at)
        ).all()
    ...
    async def rollback_parameters(
        ...
        try:
            ...
            rolled_back_versions = parameter_service.rollback_parameter(
                db, rollback_request
            )
...
def mount_parameter_management(
    app: FastAPI, db_session_factory, prefix: str = "/api/v1/parameters"
):
