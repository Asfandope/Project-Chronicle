...
class EvaluationRun(Base):
    ...
    trigger_source = Column(
        String(100)
    )  # 'drift_detection', 'manual_upload', 'scheduled'
    ...
    # Relationships
    document_evaluations = relationship(
        "DocumentEvaluation", back_populates="evaluation_run"
    )
...
class DocumentEvaluation(Base):
    ...
    evaluation_run = relationship(
        "EvaluationRun", back_populates="document_evaluations"
    )
    article_evaluations = relationship(
        "ArticleEvaluation", back_populates="document_evaluation"
    )
...
class ArticleEvaluation(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_evaluation_id = Column(
        UUID(as_uuid=True), ForeignKey("document_evaluations.id"), nullable=False
    )
    ...
    # Relationships
    document_evaluation = relationship(
        "DocumentEvaluation", back_populates="article_evaluations"
    )
...
class DriftDetection(Base):
    ...
    actions_triggered = Column(
        JSON
    )  # List of actions taken (alerts, auto-tuning, etc.)
...
class AutoTuningEvent(Base):
    ...
    # Trigger information
    trigger_drift_detection_id = Column(
        UUID(as_uuid=True), ForeignKey("drift_detections.id")
    )
    ...
    tuning_type = Column(
        String(50), nullable=False
    )  # 'model_retrain', 'parameter_adjust', etc.
...
class EvaluationMetrics(Base):
    ...
    __table_args__ = (
        Index(
            "idx_evaluation_metrics_name_period", "metric_name", "aggregation_period"
        ),
...
def get_latest_evaluation_runs(
...
def get_drift_detections_in_window(
    session: Session, metric_type: str, window_size: int = 10
) -> List[DriftDetection]:
