...
class BatchEvaluationRequest(BaseModel):
    ...
    drift_threshold: float = Field(
        0.05, ge=0.0, le=1.0, description="Drift detection threshold"
    )
...
class DriftDetectionSettings(BaseModel):
    ...
    # Thresholds
    drift_threshold: float = Field(
        0.05, ge=0.01, le=0.5, description="Drift detection threshold"
    )
    ...
    auto_tuning_threshold: float = Field(
        0.15, ge=0.05, le=0.5, description="Auto-tuning trigger threshold"
    )
    ...
    def auto_tuning_threshold_greater_than_alert(cls, v, values):
        ...
        if v <= alert_threshold:
            raise ValueError(
                "Auto-tuning threshold must be greater than alert threshold"
            )
        return v
...
class XMLValidationRequest(BaseModel):
    ...
    xml_type: str = Field(
        ..., pattern="^(ground_truth|extracted)$", description="Type of XML content"
    )
