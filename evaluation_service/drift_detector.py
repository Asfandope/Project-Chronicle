...
from .schemas import DriftStatus
...
class DriftDetector:
    ...
    def detect_drift(
        ...
        for metric_type in metric_types:
            try:
                result = self._analyze_metric_drift(
                    session, evaluation_run, metric_type
                )
                ...
                self.logger.error(
                    f"Error detecting drift for metric {metric_type}: {str(e)}"
                )
                continue
    ...
    def _analyze_metric_drift(
        self, session: Session, evaluation_run: EvaluationRun, metric_type: str
    ) -> DriftAnalysisResult:
        ...
        statistical_significance = (
            self.config.enable_statistical_tests
            and p_value is not None
            and p_value < (1 - self.config.confidence_level)
        )
    ...
    def _get_metric_window_data(
        self, session: Session, metric_type: str
    ) -> List[float]:
    ...
    def _get_current_accuracy(
        self, evaluation_run: EvaluationRun, metric_type: str
    ) -> float:
    ...
    def _calculate_baseline_accuracy(self, session: Session, metric_type: str) -> float:
        ...
        history = get_accuracy_history(
            session, metric_type, days=self.config.baseline_lookback_days
        )
        ...
        accuracies = [
            point["accuracy"] for point in history if point["accuracy"] is not None
        ]
    ...
    def _calculate_statistical_significance(
        self, window_data: List[float], current_accuracy: float
    ...
    def _generate_recommendations(
        ...
        if metric_type == "title":
            recommendations.append(
                "Review title extraction patterns and font recognition"
            )
            recommendations.append("Check for new magazine layouts or design changes")
        elif metric_type == "body_text":
            recommendations.append(
                "Analyze text extraction quality and OCR performance"
            )
        ...
        if trend_direction == "declining":
            recommendations.append(
                "Declining trend detected - investigate underlying causes"
            )
        ...
        if statistical_significance:
            recommendations.append(
                "Change is statistically significant - immediate attention required"
            )
        else:
            recommendations.append(
                "Change may be due to normal variation - continue monitoring"
            )
    ...
    def _trigger_auto_tuning(
        self, session: Session, result: DriftAnalysisResult
    ) -> None:
    ...
    def _execute_auto_tuning(
        self, session: Session, auto_tuning_event: AutoTuningEvent
    ) -> None:
        ...
        try:
            ...
            auto_tuning_event.post_tuning_accuracy = (
                auto_tuning_event.pre_tuning_accuracy + 0.02
            )  # 2% improvement
            auto_tuning_event.improvement = (
                auto_tuning_event.post_tuning_accuracy
                - auto_tuning_event.pre_tuning_accuracy
            )
            auto_tuning_event.execution_log = (
                "Auto-tuning completed successfully with simulated 2% improvement"
            )
            ...
            self.logger.error(
                f"Auto-tuning failed for event {auto_tuning_event.id}: {str(e)}"
            )
    ...
    def get_drift_summary(self, session: Session, days: int = 7) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "drift_detected_count": sum(
                1 for d in drift_detections if d.drift_detected
            ),
            "alerts_triggered_count": sum(
                1 for d in drift_detections if d.alert_triggered
            ),
            "auto_tuning_triggered_count": sum(
                1 for d in drift_detections if d.auto_tuning_triggered
            ),
            "metrics_affected": list(set(d.metric_type for d in drift_detections)),
            "average_accuracy_drop": statistics.mean(
                [d.accuracy_drop for d in drift_detections]
            )
            if drift_detections
            else 0,
            "period_days": days,
