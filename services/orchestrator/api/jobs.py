...
from shared.schemas.job import JobListResponse, JobResponse
...
async def process_pdf(
    ...
    brand: Optional[str] = Query(
        None, description="Brand identifier for processing configuration"
    ),
    priority: int = Query(
        0, description="Job priority (higher = more priority)", ge=0, le=10
    ),
    ...
    try:
        ...
        logger.error(
            "Failed to read uploaded file", error=str(e), filename=file.filename
        )
...
async def retry_job(job_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    if job.overall_status not in [WorkflowStatus.FAILED, WorkflowStatus.COMPLETED]:
        raise HTTPException(
            status_code=400, detail="Can only retry failed or quarantined jobs"
        )
