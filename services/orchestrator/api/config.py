    ...
    for filename in os.listdir(configs_dir):
        if filename.endswith(".yaml") or filename.endswith(".yml"):
            ...
            try:
                ...
                logger.error(
                    "Failed to load brand config", brand=brand_name, error=str(e)
                )
...
async def get_brand_config(brand_name: str) -> Dict[str, Any]:
    ...
    if not os.path.exists(config_path):
        raise HTTPException(
            status_code=404, detail=f"Brand configuration '{brand_name}' not found"
        )
...
async def update_brand_config(
    brand_name: str, config: Dict[str, Any]
) -> Dict[str, str]:
...
async def get_processing_config() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to load processing configuration"
        )
