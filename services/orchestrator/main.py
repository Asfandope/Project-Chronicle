...
from .api import config, health, jobs
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if settings.enable_file_watcher:
            ...
            logger.info(
                "File watcher service started", directory=settings.input_directory
            )
...
def create_app() -> FastAPI:
    ...
    async def log_requests(request: Request, call_next):
        ...
        try:
            ...
            process_time = time.time() - start_time
            logger_ctx.error(
                "Request failed", error=str(e), process_time=process_time, exc_info=True
            )
            raise
    ...
    async def internal_server_error(request: Request, exc: Exception):
        ...
        return {"detail": "Internal server error", "correlation_id": correlation_id}
