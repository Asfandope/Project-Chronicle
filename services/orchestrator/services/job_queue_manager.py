...
from orchestrator.core.workflow import WorkflowStage, WorkflowStatus
...
class JobQueueManager:
    ...
    async def get_job_status(self, job_id: UUID) -> Optional[Dict]:
        ...
        async with AsyncSessionLocal() as session:
            query = (
                select(Job)
                .options(selectinload(Job.processing_states))
                .where(Job.id == job_id)
            )
    ...
    async def retry_job(self, job_id: UUID) -> bool:
        ...
        async with AsyncSessionLocal() as session:
            ...
            logger.info(
                "Job queued for retry", job_id=str(job_id), retry_count=job.retry_count
            )
    ...
    async def _run_scheduler(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error("Error in job scheduler", error=str(e), exc_info=True)
    ...
    async def _start_job_processing(self, job: Job, session: AsyncSession) -> None:
        ...
        try:
            ...
            task = process_pdf_task.apply_async(args=[job_id], headers=headers)
    ...
    async def _cleanup_stale_jobs(self) -> None:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if completed_jobs:
                logger.info(
                    "Cleaned up completed jobs", completed_count=len(completed_jobs)
                )
    ...
    async def get_queue_stats(self) -> Dict:
        ...
        async with AsyncSessionLocal() as session:
            ...
            stats_query = select(
                Job.overall_status, func.count(Job.id).label("count")
            ...
            return {
                ...
                "queue_utilization": len(self._active_jobs)
