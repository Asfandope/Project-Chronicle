...
def main():
    ...
    try:
        ...
        print(
            f"   • Confidence adjustments: {len(layout_config.get('confidence_adjustments', {}))}"
        )
        print(
            f"   • Spatial config: {bool(layout_config.get('spatial_relationships'))}"
        )
        print(
            f"   • Post-processing rules: {bool(layout_config.get('post_processing'))}"
        )
    ...
    graph_stats = {
        "nodes": {"text_blocks": 42, "images": 3, "page_breaks": 2},
        "edges": {
            "follows": 38,
            "belongs_to": 15,
            "above": 12,
    ...
    print(
        f"   • Spatial relationships: {sum(graph_stats['edges'][k] for k in ['above', 'below', 'left_of', 'right_of'])}"
    )
    ...
    target_met = (
        "✅ TARGET MET" if overall_accuracy >= target_accuracy else "❌ BELOW TARGET"
    )
    ...
    print(
        f"   • Uncertain predictions flagged: {learning_stats['uncertain_predictions']}"
    )
    print(
        f"   • Human feedback incorporated: {learning_stats['feedback_incorporated']}"
    )
    print(
        f"   • Model improvements this session: {learning_stats['model_improvements']}"
    )
