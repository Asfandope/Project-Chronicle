...
class TuningRun(Base):
    ...
    )  # drift_detection, manual, scheduled
    trigger_accuracy_drop = Column(Float)
    trigger_metric_type = Column(String(50))

    # Status tracking
    status = Column(
        Enum(TuningStatus), nullable=False, default=TuningStatus.PENDING, index=True
    )
    ...
    optimization_strategy = Column(
        Enum(OptimizationStrategy), default=OptimizationStrategy.GRID_SEARCH
    )
    ...
    failure_patterns = relationship(
        "FailurePattern", back_populates="tuning_run", cascade="all, delete-orphan"
    )
    synthetic_datasets = relationship(
        "SyntheticDataset", back_populates="tuning_run", cascade="all, delete-orphan"
    )
    parameter_experiments = relationship(
        "ParameterExperiment", back_populates="tuning_run", cascade="all, delete-orphan"
    )
    validation_results = relationship(
        "ValidationResult", back_populates="tuning_run", cascade="all, delete-orphan"
    )
...
class FailurePattern(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tuning_run_id = Column(
        UUID(as_uuid=True), ForeignKey("tuning_runs.id"), nullable=False
    )
    created_at = Column(
        DateTime(timezone=True),
        default=lambda: datetime.now(timezone.utc),
    ...
    targeted_examples = relationship(
        "TargetedSyntheticExample", back_populates="failure_pattern"
    )
...
class SyntheticDataset(Base):
    ...
    targeted_examples = relationship(
        "TargetedSyntheticExample", back_populates="synthetic_dataset"
    )
...
class TargetedSyntheticExample(Base):
    ...
    synthetic_dataset_id = Column(
        UUID(as_uuid=True), ForeignKey("synthetic_datasets.id"), nullable=False
    )
    failure_pattern_id = Column(
        UUID(as_uuid=True), ForeignKey("failure_patterns.id"), nullable=False
    )
    ...
    synthetic_dataset = relationship(
        "SyntheticDataset", back_populates="targeted_examples"
    )
...
class ParameterExperiment(Base):
    ...
    )  # Hash of parameter set for deduplication
    ...
    parent_experiment_id = Column(
        UUID(as_uuid=True), ForeignKey("parameter_experiments.id")
    )
...
class ValidationResult(Base):
    ...
    parameter_experiment_id = Column(
        UUID(as_uuid=True), ForeignKey("parameter_experiments.id")
    )
...
def check_tuning_rate_limit(
    session: Session, brand_name: str
...
def record_tuning_run_start(
    session: Session, brand_name: str, tuning_run_id: str
) -> None:
...
def get_recent_tuning_runs(
    session: Session, brand_name: str = None, limit: int = 10
) -> List[TuningRun]:
...
def get_tuning_run_statistics(session: Session, days: int = 30) -> Dict[str, Any]:
    ...
    runs = session.query(TuningRun).filter(TuningRun.created_at >= cutoff_date).all()
    ...
    return {
        ...
        "average_duration_minutes": sum(
            r.total_duration_seconds or 0 for r in successful_runs
        )
        / len(successful_runs)
        / 60
        if successful_runs
