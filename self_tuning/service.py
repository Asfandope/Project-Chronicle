...
from evaluation_service.models import DocumentEvaluation, EvaluationRun
...
class SelfTuningService:
    ...
    def start_tuning_run(
        ...
        if not force and not self._check_rate_limit(session, brand_name):
            raise ValueError(
                f"Rate limit exceeded: max one tuning run per brand per day"
            )
        ...
        if quarantined_count < 10:
            raise ValueError(
                f"Insufficient quarantined data: {quarantined_count} issues (need ≥10)"
            )
    ...
    def identify_failure_patterns(
        self, session: Session, tuning_run: TuningRun
        ...
        quarantined_results = (
            session.query(DocumentEvaluation)
            .join(EvaluationRun)
            .filter(
                and_(
        ...
        contributor_failures = [
            r for r in quarantined_results if r.contributors_accuracy < 0.7
        ]
        ...
        media_failures = [
            r for r in quarantined_results if r.media_links_accuracy < 0.6
        ]
    ...
    def generate_targeted_synthetic_data(
        ...
        """
        self.logger.info(
            f"Generating targeted synthetic data for tuning run {tuning_run.id}"
        )
        ...
        self.logger.info(
            f"Generated {len(synthetic_examples)} targeted synthetic examples"
        )
        return dataset
    ...
    def optimize_parameters(
        ...
        """
        self.logger.info(
            f"Starting parameter optimization for tuning run {tuning_run.id}"
        )
        ...
        for param_combination in self._generate_parameter_combinations(grid_config):
            ...
            try:
                ...
                self._apply_experimental_parameters(
                    session, param_combination, tuning_run.brand_name
                )
                ...
                experiment.improvement_over_baseline = (
                ...
                self.logger.debug(
                    f"Experiment {experiment.id}: accuracy={accuracy_score:.4f}, improvement={experiment.improvement_over_baseline:.4f}"
                )

                # Early stopping if we found a good improvement
                if (
                    experiment.improvement_over_baseline
                    > grid_config.early_stopping_threshold
                ):
                    self.logger.info(
                        f"Early stopping triggered: improvement={experiment.improvement_over_baseline:.4f}"
                    )
                    break
    ...
    def validate_best_parameters(
        ...
        try:
            ...
            holdout_results = self._get_holdout_validation_set(
                session, tuning_run.brand_name
            )

            if len(holdout_results) < 10:
                raise ValueError(
                    f"Insufficient holdout data: {len(holdout_results)} samples"
                )

            # Calculate validation metrics
            holdout_accuracy = self._calculate_holdout_accuracy(holdout_results)
            baseline_accuracy = self._get_baseline_accuracy_for_validation(
                session, tuning_run.brand_name
            )
            ...
            validation.is_statistically_significant = p_value < (
    ...
    def deploy_or_rollback(
        ...
        should_deploy = (
            ...
            and validation_result.meets_improvement_threshold
            and validation_result.is_statistically_significant
        )
        ...
        if should_deploy:
            ...
            self.logger.info(
                f"Parameters deployed successfully: improvement={validation_result.accuracy_improvement:.4f}"
            )
            ...
            if not validation_result.meets_improvement_threshold:
                rollback_reasons.append(
                    f"Insufficient improvement: {validation_result.accuracy_improvement:.4f} < {self.min_improvement_threshold}"
                )
            if not validation_result.is_statistically_significant:
                rollback_reasons.append(
                    f"Not statistically significant: p={validation_result.p_value:.4f}"
                )
    ...
    def run_complete_tuning_cycle(
        ...
        try:
            ...
            tuning_run = self.start_tuning_run(
                session, brand_name, triggered_by, force=force
            )
            ...
            failure_patterns = (
                ...
                .filter(FailurePattern.tuning_run_id == tuning_run.id)
                .all()
            )
            ...
            validation_result = self.validate_best_parameters(
                session, tuning_run, experiments
            )

            # Step 6: Deploy or rollback
            final_result = self.deploy_or_rollback(
                session, tuning_run, validation_result
            )
    ...
    def _check_rate_limit(self, session: Session, brand_name: str) -> bool:
        ...
        recent_runs = (
            session.query(TuningRunRateLimit)
            .filter(
                and_(
    ...
    def _analyze_title_failures(
        self, failures: List[DocumentEvaluation]
    ) -> FailureAnalysis:
        ...
        return FailureAnalysis(
            ...
            examples=[
                {"evaluation_id": str(r.id), "accuracy": r.title_accuracy}
                for r in failures[:5]
            ],
    ...
    def _analyze_body_text_failures(
        self, failures: List[DocumentEvaluation]
    ) -> FailureAnalysis:
        ...
        return FailureAnalysis(
            ...
            frequency=len(failures),
            examples=[
                {"evaluation_id": str(r.id), "accuracy": r.body_text_accuracy}
                for r in failures[:5]
            ],
            suggested_adjustments={
    ...
    def _analyze_contributor_failures(
        self, failures: List[DocumentEvaluation]
    ) -> FailureAnalysis:
        ...
        return FailureAnalysis(
            ...
            frequency=len(failures),
            examples=[
                {"evaluation_id": str(r.id), "accuracy": r.contributors_accuracy}
                for r in failures[:5]
            ],
            suggested_adjustments={"accuracy.contributors_weight": {"increase": 0.05}},
        )

    def _analyze_media_failures(
        self, failures: List[DocumentEvaluation]
    ) -> FailureAnalysis:
        ...
        return FailureAnalysis(
            ...
            frequency=len(failures),
            examples=[
                {"evaluation_id": str(r.id), "accuracy": r.media_links_accuracy}
                for r in failures[:5]
            ],
            suggested_adjustments={"accuracy.media_links_weight": {"increase": 0.05}},
        )

    def _create_failure_pattern(
        self, session: Session, tuning_run: TuningRun, analysis: FailureAnalysis
    ) -> FailurePattern:
    ...
    def _build_generation_config(
        ...
        config = {"focus_areas": [], "edge_cases": [], "variations": {}}
    ...
    def _generate_examples_for_pattern(
        self, pattern: FailurePattern, count: int, brand_name: str
    ) -> List[Dict[str, str]]:
    ...
    def _build_grid_search_config(
        ...
        return GridSearchConfig(
            parameters=parameters, max_experiments=50, early_stopping_threshold=0.02
        )
    ...
    def _calculate_baseline_accuracy(
        self, session: Session, brand_name: str, synthetic_dataset: SyntheticDataset
    ) -> float:
    ...
    def _apply_experimental_parameters(
        self, session: Session, parameters: Dict[str, Any], brand_name: str
    ):
    ...
    def _evaluate_parameter_combination(
        self, session: Session, synthetic_dataset: SyntheticDataset, brand_name: str
    ) -> float:
    ...
    def _calculate_holdout_accuracy(
        self, holdout_results: List[DocumentEvaluation]
    ) -> float:
    ...
    def _get_baseline_accuracy_for_validation(
        self, session: Session, brand_name: str
    ) -> float:
    ...
    def _calculate_statistical_significance(
        self, new_accuracy: float, baseline_accuracy: float, sample_size: int
