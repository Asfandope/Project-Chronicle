...
class ContentFactory:
    ...
    def _select_article_template(
        self, brand_config: BrandConfiguration, article_type: Optional[str]
    ) -> ArticleTemplate:
        ...
        if article_type:
            ...
            matching_templates = [
                template
                for template in self.article_templates.values()
        ...
        preferred_templates = brand_preferred.get(
            brand_config.brand_style, list(self.article_templates.keys())
        )
    ...
    def _generate_contributors(
        self, template: ArticleTemplate, edge_cases: List[EdgeCaseType]
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_text_content(
        ...
        if complexity_level == "simple":
            paragraph_count = random.randint(
                min_paragraphs, (min_paragraphs + max_paragraphs) // 2
            )
        elif complexity_level == "complex":
            paragraph_count = random.randint(
                (min_paragraphs + max_paragraphs) // 2, max_paragraphs
            )
        else:  # moderate
        ...
        content_pool = self.sample_content.get(
            template.content_themes[0].split()[0], self.sample_content["business"]
        )
    ...
    def _generate_caption(
        self, media_template: MediaTemplate, article_template: ArticleTemplate
    ) -> str:
