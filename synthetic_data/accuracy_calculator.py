...
class DocumentAccuracy:
    ...
    def _calculate_document_accuracy(self):
        ...
        total_weighted = sum(
            article.weighted_overall_accuracy for article in self.article_accuracies
        )
...
class AccuracyCalculator:
    ...
    def _calculate_title_accuracy(
        self, ground_truth_title: str, extracted_title: str
    ) -> FieldAccuracy:
    ...
    def _calculate_body_text_accuracy(
        self, ground_truth_article: ArticleData, extracted_text: str
    ) -> FieldAccuracy:
        ...
        body_elements = [
            elem
            for elem in ground_truth_article.text_elements
    ...
    def _calculate_contributors_accuracy(
        ...
        for gt_contrib in ground_truth_contributors:
            ...
            for ex_contrib in extracted_contributors:
                ...
                if name_match and role_match:
                    ...
                    name_similarity = 1.0
                    role_similarity = difflib.SequenceMatcher(
                        None, gt_role, ex_role
                    ).ratio()
        ...
        accuracy = (
            correct_matches / total_contributors if total_contributors > 0 else 1.0
        )
    ...
    def _calculate_media_links_accuracy(
        self, ground_truth_article: ArticleData, extracted_media: List[Dict[str, Any]]
    ) -> FieldAccuracy:
            ...
            for ex_media in extracted_media:
                ...
                if gt_caption:
                    ...
                    caption_score = difflib.SequenceMatcher(
                        None, gt_caption.lower().strip(), ex_caption.lower().strip()
                    ).ratio()
                else:
    ...
    def _find_associated_caption(
        self, image_element: ImageElement, article: ArticleData
    ) -> Optional[str]:
        ...
        caption_elements = [
            elem
            for elem in article.text_elements
            if elem.semantic_type == "caption"
            and elem.page_number == image_element.page_number
        ]
        ...
        for caption in caption_elements:
            ...
            distance = (
                (image_center_x - caption_center_x) ** 2
                + (image_center_y - caption_center_y) ** 2
            ) ** 0.5
    ...
    def _compare_image_properties(
        self, gt_image: ImageElement, extracted_media: Dict[str, Any]
    ) -> float:
    ...
    def calculate_document_accuracy(
        self, ground_truth: GroundTruthData, extracted_document: Dict[str, Any]
    ) -> DocumentAccuracy:
        ...
        overall_title = self._aggregate_field_accuracy(
            article_accuracies, "title_accuracy"
        )
        overall_body = self._aggregate_field_accuracy(
            article_accuracies, "body_text_accuracy"
        )
        overall_contributors = self._aggregate_field_accuracy(
            article_accuracies, "contributors_accuracy"
        )
        overall_media = self._aggregate_field_accuracy(
            article_accuracies, "media_links_accuracy"
        )
    ...
    def _find_matching_article(
        self, gt_article: ArticleData, extracted_articles: List[Dict[str, Any]]
    ...
    def _aggregate_field_accuracy(
        self, article_accuracies: List[ArticleAccuracy], field_attr: str
    ) -> FieldAccuracy:
        ...
        total_correct = sum(
            getattr(acc, field_attr).correct for acc in article_accuracies
        )
        total_count = sum(getattr(acc, field_attr).total for acc in article_accuracies)
...
def create_accuracy_report(
    document_accuracy: DocumentAccuracy, output_path: Optional[Path] = None
) -> Dict[str, Any]:
        ...
        "field_accuracies": {
            "title": {
                ...
                "accuracy_percentage": round(
                    document_accuracy.overall_title_accuracy.accuracy_percentage, 2
                ),
            ...
            "body_text": {
                "weight": 40,
                "accuracy_percentage": round(
                    document_accuracy.overall_body_text_accuracy.accuracy_percentage, 2
                ),
            ...
            "contributors": {
                "weight": 20,
                "accuracy_percentage": round(
                    document_accuracy.overall_contributors_accuracy.accuracy_percentage,
                    2,
                ),
            ...
            "media_links": {
                "weight": 10,
                "accuracy_percentage": round(
                    document_accuracy.overall_media_links_accuracy.accuracy_percentage,
                    2,
                ),
        ...
        "article_details": [
            {
                ...
                "field_accuracies": {
                    ...
                    "contributors": round(
                        acc.contributors_accuracy.accuracy_percentage, 2
                    ),
                    "media_links": round(
                        acc.media_links_accuracy.accuracy_percentage, 2
                    ),
                },
            }
