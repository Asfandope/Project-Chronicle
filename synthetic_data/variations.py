...
class VariationEngine:
    ...
    def apply_font_variations(
        ...
        try:
            ...
            self.logger.debug(
                "Font variations applied",
                text_elements_modified=sum(
                    1 for e in varied_elements if isinstance(e, TextElement)
                ),
            )
    ...
    def _create_font_variation(
        ...
        # Style variation
        style_variation = (
            "italic" if random.random() < 0.2 * variation_strength else "normal"
        )
    ...
    def _apply_font_variation(
        self, element: TextElement, variation: FontVariation
    ) -> TextElement:
    ...
    def _apply_decorative_title_variation(
        self, elements: List[LayoutElement], brand_config: BrandConfiguration
    ) -> List[LayoutElement]:
    ...
    def _apply_overlapping_elements_variation(
        self, elements: List[LayoutElement], page_size: Tuple[float, float]
    ) -> List[LayoutElement]:
    ...
    def _apply_rotated_text_variation(
        ...
        for element in elements:
            if (
                ...
                and element.semantic_type in ["sidebar", "caption", "byline"]
    ...
    def _resolve_overlaps_if_needed(
        self, elements: List[LayoutElement], overlap_probability: float
    ) -> List[LayoutElement]:
    ...
    def get_variation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "color_variations_applied": self.variation_stats[
                "color_variations_applied"
            ],
            "layout_variations_applied": self.variation_stats[
                "layout_variations_applied"
            ],
            "edge_case_variations_applied": self.variation_stats[
                "edge_case_variations_applied"
            ],
            "total_variations": sum(self.variation_stats.values()),
            "font_families_available": sum(
                len(fonts) for fonts in self.font_families.values()
            ),
            "color_palettes_available": sum(
                len(palettes) for palettes in self.color_palettes.values()
            ),
        }
