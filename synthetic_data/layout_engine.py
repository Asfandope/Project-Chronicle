...
class MagazineStyle:
    ...
    # Layout preferences
    preferred_image_sizes: List[Tuple[float, float]] = field(
    ...
    def create_tech_style(cls) -> "MagazineStyle":
        ...
        return cls(
            ...
            preferred_image_sizes=[(250, 180), (320, 240), (400, 250), (180, 120)],
        )
    ...
    def create_fashion_style(cls) -> "MagazineStyle":
        ...
        return cls(
            ...
            preferred_image_sizes=[(300, 400), (250, 350), (400, 500), (200, 300)],
        )
...
class LayoutTemplate:
    ...
    image_placement_options: List[str] = field(
        default_factory=lambda: ["top", "middle", "bottom", "side"]
    )
...
class LayoutEngine:
    ...
    def generate_page_layout(
        ...
        try:
            ...
            page_size = self.page_sizes.get(
                brand_config.brand_style.value.upper() + "_SIZE",
    ...
    def _select_template(
        self, complexity: Optional[LayoutComplexity], edge_cases: List[EdgeCaseType]
    ) -> LayoutTemplate:
        ...
        if edge_cases:
            ...
            for template in suitable_templates:
                if (
                    EdgeCaseType.DECORATIVE_TITLES in edge_cases
                    and not template.supports_decorative_titles
                ):
                    continue
                if (
                    EdgeCaseType.OVERLAPPING_ELEMENTS in edge_cases
                    and not template.supports_overlapping_elements
                ):
                    continue
                if (
                    EdgeCaseType.SPLIT_ARTICLES in edge_cases
                    and not template.supports_split_articles
                ):
                    continue
    ...
    def _generate_layout_elements(
        ...
        if content_requirements.get("contributors"):
            byline_elements = self._create_byline_elements(
                ...
                magazine_style,
                page_number,
                EdgeCaseType.CONTRIBUTOR_COMPLEXITY in edge_cases,
            )
            ...
            if byline_elements:
                ...
                column_y_positions = [
                    max(pos, byline_bottom) for pos in column_y_positions
                ]
        ...
        for i, text_block in enumerate(text_blocks):
            ...
            column_y_positions[column_idx] = (
                text_element.bbox[3] + magazine_style.paragraph_spacing
            )
    ...
    def _create_title_element(
        ...
        title_element = TextElement(
            ...
            font_style="bold"
            if not decorative
            else random.choice(["bold", "bold-italic"]),
            semantic_type="title",
            reading_order=1,
    ...
    def _select_image_placement(
        self, template: LayoutTemplate, image_index: int, total_images: int
    ) -> str:
