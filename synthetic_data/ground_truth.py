...
class GroundTruthGenerator:
    ...
    def generate_ground_truth(
        ...
        max_page = max(
            [elem.page_number for elem in all_text_elements + all_image_elements],
            default=1,
        )
    ...
    def export_to_xml(
        ...
        try:
            ...
            ET.SubElement(
                doc_meta, "generation_timestamp"
            ).text = ground_truth.generation_timestamp.isoformat()
            ...
            ET.SubElement(quality, "expected_accuracy").text = str(
                ground_truth.expected_extraction_accuracy
            )
            ET.SubElement(quality, "difficult_elements").text = str(
                ground_truth.difficult_elements_count
            )
    ...
    def _add_article_to_xml(self, parent: ET.Element, article: ArticleData) -> None:
        ...
        if article.is_split_article:
            ...
            if article.continuation_pages:
                page_range.set(
                    "continuation_pages", ",".join(map(str, article.continuation_pages))
                )
    ...
    def _add_text_element_to_xml(
        self, parent: ET.Element, element: TextElement
    ) -> None:
    ...
    def _add_image_element_to_xml(
        self, parent: ET.Element, element: ImageElement
    ) -> None:
    ...
    def export_to_json(self, ground_truth: GroundTruthData, output_path: Path) -> bool:
        ...
        try:
            data = {
                ...
                "articles": [
                    self._article_to_dict(article) for article in ground_truth.articles
                ],
                "all_text_elements": [
                    self._text_element_to_dict(elem)
                    for elem in ground_truth.all_text_elements
                ],
                "all_image_elements": [
                    self._image_element_to_dict(elem)
                    for elem in ground_truth.all_image_elements
                ],
            }
    ...
    def _article_to_dict(self, article: ArticleData) -> Dict[str, Any]:
        ...
        return {
            ...
            "page_range": {
                "start": article.page_range[0],
                "end": article.page_range[1],
            },
            ...
            "text_elements": [
                self._text_element_to_dict(elem) for elem in article.text_elements
            ],
            "image_elements": [
                self._image_element_to_dict(elem) for elem in article.image_elements
            ],
        }
    ...
    def _text_element_to_dict(self, element: TextElement) -> Dict[str, Any]:
        ...
        return {
            ...
            "color": {
                "r": element.text_color[0],
                "g": element.text_color[1],
                "b": element.text_color[2],
            },
            "extraction_metadata": {
...
class ConfidenceCalculator:
    ...
    def calculate_document_confidence(
        self, elements: List[Any], edge_cases: List[EdgeCaseType] = None
    ) -> float:
        ...
        for element in elements:
            ...
            overlapping = [
                ...
                if other != element
                and hasattr(element, "overlaps")
                and element.overlaps(other)
            ]

            confidence = self.calculate_element_confidence(
                element, overlapping, edge_cases
            )
    ...
    def calculate_difficulty_distribution(self, elements: List[Any]) -> Dict[str, int]:
...
def create_test_ground_truth(output_dir: Path) -> Path:
    ...
    from .types import ArticleData, BrandConfiguration, TextElement
    ...
    ground_truth = generator.generate_ground_truth(
        [test_article], brand_config, document_id="test_document"
    )
