...
class PDFRenderer:
    ...
    def _render_page(
        ...
        page_text_elements = [
            elem for elem in article.text_elements if elem.page_number == logical_page
        ]
        page_image_elements = [
            elem for elem in article.image_elements if elem.page_number == logical_page
        ]
    ...
    def _render_image_element(
        self, canvas: "canvas.Canvas", element: ImageElement
    ) -> None:
    ...
    def _wrap_text(
        self, canvas: "canvas.Canvas", text: str, width: float, font_size: float
    ) -> List[str]:
    ...
    def _add_metadata(
        ...
        if generation_config:
            canvas.setKeywords(
                f"synthetic,test,magazine,{brand_config.brand_style.value}"
            )

    def _draw_debug_grid(
        self, canvas: "canvas.Canvas", width: float, height: float
    ) -> None:
    ...
    def _draw_crop_marks(
        ...
        for corner_x, corner_y in corners:
            ...
            if corner_x == 0:
                canvas.line(
                    -mark_offset - mark_length, corner_y, -mark_offset, corner_y
                )
            else:
                canvas.line(
                    corner_x + mark_offset,
                    corner_y,
                    corner_x + mark_offset + mark_length,
                    corner_y,
                )
            ...
            if corner_y == 0:
                canvas.line(
                    corner_x, -mark_offset - mark_length, corner_x, -mark_offset
                )
            else:
                canvas.line(
                    corner_x,
                    corner_y + mark_offset,
                    corner_x,
                    corner_y + mark_offset + mark_length,
                )

    def _add_generation_watermark(
        self, canvas: "canvas.Canvas", width: float, height: float
    ) -> None:
...
def create_sample_document(output_dir: Path) -> Path:
    ...
    body_element = TextElement(
        ...
        bbox=(72, 400, 540, 600),
        page_number=1,
        text_content="This is sample body text for testing the PDF rendering system. "
        * 20,
