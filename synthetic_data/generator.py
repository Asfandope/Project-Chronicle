...
class SyntheticDataGenerator:
    ...
    def __init__(self, generation_config: GenerationConfig):
        ...
        self.pdf_renderer = PDFRenderer(
            RenderingOptions.create_high_quality()
            if self.config.pdf_dpi > 200
        ...
        logger.info(
            f"Initialized synthetic data generator with {len(self.brand_configs)} brands"
        )
    ...
    def generate_complete_test_suite(self) -> TestSuite:
        ...
        try:
            ...
            for brand_config in self.brand_configs:
                logger.info(
                    f"Generating documents for brand: {brand_config.brand_name}"
                )
                ...
                logger.info(
                    f"Generated {len(brand_documents)} documents for {brand_config.brand_name}"
                )
            ...
            end_time = time.time()
            logger.info(
                f"Test suite generation completed in {end_time - start_time:.2f} seconds"
            )
            logger.info(f"Generated {suite.total_documents} total documents")
            logger.info(
                f"Success rate: {suite.successful_generations / max(1, suite.total_documents):.2%}"
            )
    ...
    def _generate_brand_documents(
        self, brand_config: BrandConfiguration
    ) -> List[GeneratedDocument]:
        """Generate all documents for a specific brand."""
        documents = []

        # Determine complexity distribution
        complexity_counts = self._calculate_complexity_distribution(
            self.config.documents_per_brand
        )
            ...
            for i in range(count):
                ...
                try:
                    document = self._generate_single_document(
                        document_id, brand_config, complexity
                    )
    ...
    def _calculate_complexity_distribution(
        ...
        for (
            complexity,
            probability,
        ) in self.config.layout_complexity_distribution.items():
    ...
    def _generate_single_document(
        ...
        logger.debug(
            f"Generating {document_id}: {page_count} pages, complexity={complexity.value}"
        )

        # Generate magazine style
        self._create_magazine_style(brand_config, complexity)

        # Generate articles
        articles = self._generate_articles_for_document(
            brand_config, page_count, complexity, edge_cases
        )

        # Apply variations
        articles = self.variation_engine.apply_variations(
            articles, brand_config, complexity, edge_cases
        )
        ...
        ground_truth = self.ground_truth_generator.generate_ground_truth(
            articles, brand_config, self.config, document_id
        )
        ...
        if self.config.validate_ground_truth:
            ...
            if validation_errors:
                ...
                if validation_errors:
                    logger.warning(
                        f"Ground truth validation errors for {document_id}: {validation_errors}"
                    )
        ...
        if self.config.generate_pdfs and success:
            try:
                pdf_success = self.pdf_renderer.render_document(
                    articles, brand_config, pdf_path, self.config
                )
                if not pdf_success:
                    success = False
        ...
        document = GeneratedDocument(
            ...
            ground_truth_path=gt_xml_path
            if self.config.generate_ground_truth
            else None,
    ...
    def _create_magazine_style(
        self, brand_config: BrandConfiguration, complexity: LayoutComplexity
    ) -> MagazineStyle:
    ...
    def generate_brand_focused_suite(
        self, brand_name: str, document_count: int = 200
    ) -> TestSuite:
    ...
    def validate_generation_setup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_doc = self._generate_single_document(
                "validation_test", test_brand, LayoutComplexity.SIMPLE
            )
            ...
            validation_results["test_generation"] = {
                ...
                "has_ground_truth": test_doc.ground_truth_path
                and test_doc.ground_truth_path.exists(),
            ...
            validation_results["test_generation"] = {"success": False, "error": str(e)}
...
def create_comprehensive_test_suite(
    output_dir: Path, documents_per_brand: int = 150
) -> TestSuite:
...
def create_edge_case_test_suite(
    output_dir: Path, documents_per_brand: int = 100
) -> TestSuite:
