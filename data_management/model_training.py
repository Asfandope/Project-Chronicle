...
class LayoutLMTrainer:
    ...
    def _parse_xml_to_examples(self, xml_file: Path) -> List[TrainingExample]:
        ...
        try:
                ...
                if contributors is not None:
                    for contrib in contributors.findall("contributor"):
                        ...
                        if name:
                            ...
                            labels.extend(
                                [self.label_mapping["byline"]] * len(contrib_tokens)
                            )
                ...
                if images is not None:
                    for img in images.findall("image"):
                        ...
                        if caption_elem is not None and caption_elem.text:
                            ...
                            labels.extend(
                                [self.label_mapping["caption"]] * len(caption_tokens)
                            )
            ...
            self.logger.error(
                "Error parsing XML file", file=str(xml_file), error=str(e)
            )
            return []
    ...
    def prepare_model_and_processor(self):
        ...
        try:
            ...
            self.processor = LayoutLMv3Processor.from_pretrained(
                self.config.model_name, apply_ocr=False
            )
    ...
    def create_datasets(self) -> Tuple[MagazineLayoutDataset, MagazineLayoutDataset]:
        ...
        if not self.training_examples:
            raise ValueError(
                "No training examples loaded. Call load_training_data() first."
            )
        ...
        # Create datasets
        train_dataset = MagazineLayoutDataset(
            train_examples, self.processor, self.config.max_sequence_length
        )
        val_dataset = MagazineLayoutDataset(
            val_examples, self.processor, self.config.max_sequence_length
        )

        self.logger.info(
            "Datasets created", train_size=len(train_dataset), val_size=len(val_dataset)
        )
    ...
    def evaluate_model(self, model_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        report = classification_report(
            all_labels, all_predictions, target_names=label_names, output_dict=True
        )
...
def create_training_config(brand: str, **kwargs) -> TrainingConfig:
