...
from .nodes import BaseNode, ImageNode, PageBreakNode, TextBlockNode
from .types import (
    EdgeType,
    GraphEdgeData,
    GraphError,
...
class SemanticGraph:
    ...
    def __init__(
        self, graph_id: Optional[str] = None, document_path: Optional[str] = None
    ):
        """
        ...
        self.logger = logger.bind(component="SemanticGraph", graph_id=self.graph_id[:8])
    ...
    def add_node(self, node: BaseNode) -> str:
        ...
        try:
            ...
            self.graph.add_node(node.node_id, **node_data.to_dict())
    ...
    def get_node(self, node_id: str) -> Optional[BaseNode]:
        ...
        try:
            ...
            raise GraphError(
                f"Failed to get node {node_id}: {e}", self.graph_id, node_id
            )
    ...
    def remove_node(self, node_id: str) -> bool:
        ...
        try:
            ...
            raise GraphError(
                f"Failed to remove node {node_id}: {e}", self.graph_id, node_id
            )
    ...
    def get_nodes_by_type(self, node_type: NodeType) -> List[BaseNode]:
        ...
        try:
            ...
            self.logger.error(
                "Error getting nodes by type", node_type=node_type.value, error=str(e)
            )
            raise GraphError(
                f"Failed to get nodes by type {node_type}: {e}", self.graph_id
            )
    ...
    def get_nodes_by_page(self, page_num: int) -> List[BaseNode]:
        ...
        try:
            ...
            self.logger.error(
                "Error getting nodes by page", page_num=page_num, error=str(e)
            )
            raise GraphError(
                f"Failed to get nodes for page {page_num}: {e}", self.graph_id
            )
    ...
    def get_edges_by_type(self, edge_type: EdgeType) -> List[GraphEdgeData]:
        ...
        try:
            ...
            self.logger.error(
                "Error getting edges by type", edge_type=edge_type.value, error=str(e)
            )
            raise GraphError(
                f"Failed to get edges by type {edge_type}: {e}", self.graph_id
            )
    ...
    def get_successors(
        ...
        try:
            ...
            self.logger.error(
                "Error getting successors", node_id=node_id[:8], error=str(e)
            )
            raise GraphError(
                f"Failed to get successors for {node_id}: {e}", self.graph_id
            )
    ...
    def get_predecessors(
        ...
        try:
            ...
            self.logger.error(
                "Error getting predecessors", node_id=node_id[:8], error=str(e)
            )
            raise GraphError(
                f"Failed to get predecessors for {node_id}: {e}", self.graph_id
            )
    ...
    def get_reading_order(self, page_num: Optional[int] = None) -> List[str]:
        ...
        try:
            ...
            if follows_graph.nodes():
                ...
                nodes = (
                    self.get_nodes_by_page(page_num)
                    if page_num
            ...
            self.logger.error(
                "Error getting reading order", page_num=page_num, error=str(e)
            )
    ...
    def from_serialized(cls, serialized: SerializedGraph) -> "SemanticGraph":
        ...
        try:
            graph = cls(
                graph_id=serialized.graph_id, document_path=serialized.document_path
            )
    ...
    def load_json(cls, file_path: Union[str, Path]) -> "SemanticGraph":
        ...
        try:
            ...
            logger.error(
                "Error loading graph JSON", file_path=str(file_path), error=str(e)
            )
