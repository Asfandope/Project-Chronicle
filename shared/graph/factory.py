...
from ..layout.types import BlockType, LayoutResult, TextBlock
...
class GraphFactory:
    ...
    def from_layout_result(
        layout_result: LayoutResult, include_page_breaks: bool = True
    ) -> SemanticGraph:
        """
        ...
        graph = SemanticGraph(document_path=str(layout_result.pdf_path))
        ...
        for page in layout_result.pages:
            ...
            if include_page_breaks and page.page_num > 1:
                page_break = PageBreakNode(
                    page_num=page.page_num, break_type="standard"
                )
            ...
            for i in range(len(page_nodes) - 1):
                graph.add_edge(
                    page_nodes[i], page_nodes[i + 1], EdgeType.FOLLOWS, confidence=0.8
                )
    ...
    def create_article_structure(
        ...
        for i, block in enumerate(body_blocks):
            ...
            if prev_node_id:
                graph.add_edge(
                    prev_node_id, body_node.node_id, EdgeType.FOLLOWS, confidence=0.8
                )
    ...
    def add_image_with_caption(
        ...
        image_node = ImageNode(
            bbox=image_bbox, page_num=page_num, image_path=image_path, confidence=1.0
        )
        ...
        graph.add_edge(caption_id, image_id, EdgeType.CAPTION_OF, confidence=0.9)

        logger.debug(
            "Added image with caption", image_id=image_id[:8], caption_id=caption_id[:8]
        )
    ...
    def create_multi_column_layout(
        columns: List[List[TextBlock]], page_num: int = 1
    ) -> SemanticGraph:
        """
    ...
    @staticmethod
    def _get_first_node_in_column(
        graph: SemanticGraph, column_blocks: List[TextBlock], column_idx: int
    ) -> Optional[str]:
        ...
        for node_id in graph.graph.nodes():
            ...
            if (
                node_attrs.get("column") == column_idx
                and node_attrs.get("text") == first_block.text
            ):
                return node_id
    ...
    @staticmethod
    def add_hierarchical_headings(
        graph: SemanticGraph, heading_hierarchy: List[Tuple[str, int, List[str]]]
    ) -> List[str]:
        """
        ...
        for heading_text, level, child_ids in heading_hierarchy:
            ...
            for child_id in child_ids:
                if child_id in graph:
                    graph.add_edge(
                        child_id, heading_id, EdgeType.BELONGS_TO, confidence=0.8
                    )

        logger.debug("Added hierarchical headings", heading_count=len(heading_nodes))
