...
class ArticleReconstructor:
    ...
    def reconstruct_articles(
        self, graph: SemanticGraph, max_articles: Optional[int] = None
        ...
        try:
            ...
            if reconstructed_articles:
                avg_confidence = sum(
                    a.reconstruction_confidence for a in reconstructed_articles
                ) / len(reconstructed_articles)
            ...
            self.logger.error(
                "Error in article reconstruction", error=str(e), exc_info=True
            )
            raise ReconstructionError(f"Failed to reconstruct articles: {e}")

    def _reconstruct_single_article(
        self, start_node_id: str, graph: SemanticGraph, visited_nodes: Set[str]
        ...
        try:
            ...
            path = self.traversal.traverse_article(
                start_node_id, graph, visited_nodes.copy()
            )
        ...
        try:
            for i, node_id in enumerate(path.node_ids):
                ...
                component = {
                    ...
                    "block_type": node_data.classification.value
    ...
    def _calculate_reconstruction_confidence(
        self, path: TraversalPath, components: List[Dict[str, Any]]
    ) -> float:
        ...
        try:
            ...
            if components:
                ...
                component_confidence = sum(component_confidences) / len(
            ...
            completeness_factor = min(
                1.0, path.path_length / 10
            )  # Normalize to 10 components
    ...
    def _meets_quality_criteria(self, article: ReconstructedArticle) -> bool:
        ...
        try:
            ...
            if self.config.filter_advertisements and any(
                "advertisement" in c.get("block_type", "") for c in article.components
            ):
                return False
    ...
    def _post_process_articles(
        self, articles: List[ReconstructedArticle], graph: SemanticGraph
    ) -> List[ReconstructedArticle]:
    ...
    def _remove_duplicate_articles(
        self, articles: List[ReconstructedArticle]
    ) -> List[ReconstructedArticle]:
        """Remove duplicate articles based on content similarity."""
        if len(articles) <= 1:
            return articles
    ...
    def _are_duplicate_articles(
        self, article1: ReconstructedArticle, article2: ReconstructedArticle
    ) -> bool:
