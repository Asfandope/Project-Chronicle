...
from .types import FilenameFormat, ImageCaptionPair, MatchingError, SpatialConfig
...
class FilenameGenerator:
    ...
    def __init__(
        ...
        strategy: Optional[FilenameStrategy] = None,
    ):
        """
        ...
        try:
            ...
            for i, pair in enumerate(sorted_pairs):
                try:
                    ...
                    fallback_filename = self._generate_fallback_filename(
                        i + self.strategy.sequence_start
                    )
            ...
            self.logger.error(
                "Error in filename generation", error=str(e), exc_info=True
            )
            raise MatchingError(f"Filename generation failed: {e}")

    def _sort_pairs_for_naming(
        self, pairs: List[ImageCaptionPair], graph: SemanticGraph
    ) -> List[ImageCaptionPair]:
    ...
    def _generate_single_filename(
        ...
        if format_type == FilenameFormat.SEQUENTIAL:
            ...
            return self._generate_hybrid_filename(
                pair, graph, sequence_number, article_context
            )
        elif format_type == FilenameFormat.ARTICLE_BASED:
            return self._generate_article_based_filename(
                pair, graph, sequence_number, article_context
            )
        else:
            return self._generate_sequential_filename(sequence_number)
    ...
    def _generate_article_based_filename(
        ...
        if article_context and self.strategy.include_article_context:
            ...
            if article_title:
                ...
                if title_words:
                    components.extend(
                        title_words[: self.strategy.article_name_max_words]
                    )
        ...
        # Join components
        base_filename = (
            "_".join(components) if components else f"img_{sequence_number:03d}"
        )
        base_filename = self._sanitize_filename(base_filename)
    ...
    def _extract_contributor_names(self, caption_text: str) -> List[str]:
        ...
        by_pattern = re.compile(
            r"\b(?:photo\s+)?by\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)", re.IGNORECASE
        )
        ...
        credit_pattern = re.compile(
            r"\b(?:photographer|illustrator|artist)\s*:?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
            re.IGNORECASE,
        )
        ...
        for name in names:
            ...
            if (
                ...
                and cleaned_name.lower() not in seen_names  # Max 3 words for a name
            ):
    ...
    def _extract_descriptive_keywords(self, caption_text: str) -> List[str]:
        ...
        text = re.sub(
            r"^(?:photo|image|picture|illustration)\s*(?:by|of|shows?)?\s*:?\s*",
            "",
            caption_text,
            flags=re.IGNORECASE,
        )
        ...
        text = re.sub(
            r"\b(?:photo|image)\s+(?:by|courtesy|credit)\s+[^.]*",
            "",
            text,
            flags=re.IGNORECASE,
        )
        ...
        for word in words:
            ...
            if (
                word_lower not in self.stop_words
                and word_lower not in photo_terms
    ...
    def _sanitize_filename(self, filename: str) -> str:
        ...
        if (
            len(filename) > self.strategy.max_length - 10
