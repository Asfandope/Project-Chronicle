...
class CaptionMatchingOrchestrator:
    ...
    def process_document(
        ...
        try:
            ...
            if (
                not initial_matches.successful_pairs
                and not initial_matches.ambiguous_matches
            ):
            ...
            if hasattr(initial_matches, "all_spatial_matches"):
                ...
                all_spatial_matches = self._extract_spatial_matches_from_result(
                    initial_matches, graph
                )

            resolved_result = self.ambiguity_resolver.resolve_ambiguities(
                all_spatial_matches, graph
            )
            ...
            optimized_result = self._apply_performance_optimizations(
                resolved_result, graph, ground_truth
            )

            # Step 5: Validate and assess quality
            final_result = self._validate_and_assess_result(
                optimized_result, ground_truth
            )
            ...
            self.logger.error(
                "Error in document processing", error=str(e), exc_info=True
            )
    ...
    def _extract_spatial_matches_from_result(
        self, result: MatchingResult, graph: SemanticGraph
    ) -> List:
    ...
    def _optimize_for_accuracy(
        ...
        self.logger.debug(
            ...
            original_pairs=len(result.successful_pairs)
            + len(result.unmatched_images)
            - len(high_confidence_pairs),
    ...
    def _optimize_for_confidence(
        ...
        for pair in result.successful_pairs:
            ...
            if (
                spatial_match.match_confidence.match_type.value
                in ["direct_below", "direct_above"]
                and spatial_match.proximity_score.overall_score >= 0.8
            ):
                ...
                spatial_match.match_confidence.overall_confidence = (
    ...
    def _optimize_for_coverage(
        ...
        if result.unmatched_images:
            ...
            for match in additional_matches:
                if (
                    hasattr(match, "match_confidence")
                    and match.match_confidence.overall_confidence >= 0.8
                ):
    ...
    def _calculate_performance_metrics(
        self, result: MatchingResult, ground_truth: Optional[Dict[str, any]] = None
    ) -> Dict[str, float]:
        """Calculate current performance metrics."""
        metrics = {
    ...
    def _matches_ground_truth(
        self, pair: ImageCaptionPair, ground_truth: Dict[str, any]
    ) -> bool:
        ...
        for correct_pair in correct_pairs:
            if (
                correct_pair.get("image_id") == pair.image_node_id
                and correct_pair.get("caption_id") == pair.caption_node_id
            ):
                return True
    ...
    def _validate_and_assess_result(
        ...
        for i, pair in enumerate(result.successful_pairs):
            if pair.filename in seen_filenames:
                ...
                extension = (
                    pair.filename.rsplit(".", 1)[1] if "." in pair.filename else "jpg"
                )
    ...
    def _update_processing_statistics(
        self, result: MatchingResult, processing_time: float
    ):
        ...
        self.stats["average_processing_time"] = (
            current_avg_time * (total_processed - 1) + processing_time
        ) / total_processed
        ...
        self.stats["average_accuracy"] = (
            current_avg_accuracy * (total_processed - 1) + current_accuracy
        ) / total_processed
    ...
    def calibrate_for_target_performance(
        ...
        try:
            ...
            avg_accuracy = sum(r["accuracy"] for r in calibration_results) / len(
                calibration_results
            )
            avg_confidence = sum(r["confidence"] for r in calibration_results) / len(
                calibration_results
            )
            avg_coverage = sum(r["coverage"] for r in calibration_results) / len(
                calibration_results
            )
            ...
            if avg_accuracy < self.target_metrics["accuracy_target"]:
                ...
                self.config.min_overall_confidence = min(
                    0.95, self.config.min_overall_confidence + 0.05
                )
                self.config.min_spatial_confidence = min(
                    0.95, self.config.min_spatial_confidence + 0.05
                )
            ...
            if avg_coverage < self.target_metrics["coverage_target"]:
                ...
                if (
                    avg_accuracy > self.target_metrics["accuracy_target"] * 1.02
                ):  # Some buffer
                    self.config.min_overall_confidence = max(
                        0.75, self.config.min_overall_confidence - 0.02
                    )
