...
class AmbiguityResolver:
    ...
    def resolve_ambiguities(
        self, matches: List[SpatialMatch], graph: SemanticGraph
    ) -> MatchingResult:
        """
        ...
        try:
            self.logger.info(
                "Starting ambiguity resolution", total_matches=len(matches)
            )
            ...
            for group in conflict_groups:
                if self._is_ambiguous_group(group):
                    ...
                    pairs = [
                        self._create_image_caption_pair(match, graph) for match in group
                    ]
            ...
            result = self._create_final_result(
                resolved_pairs, ambiguous_cases, matches, graph
            )
    ...
    def _identify_conflicts(
        ...
        for match in matches:
            ...
            while to_process:
                ...
                for related_match in image_to_matches.get(
                    current_match.image_node_id, []
                ):
                    if id(related_match) not in processed_matches:
                        to_process.append(related_match)

                # Add all matches for the same caption
                for related_match in caption_to_matches.get(
                    current_match.caption_node_id, []
                ):
                    if id(related_match) not in processed_matches:
                        to_process.append(related_match)
    ...
    def _resolve_conflict_group(
        self, group: List[SpatialMatch], graph: SemanticGraph
    ) -> Dict[str, List]:
        ...
        for strategy in strategies:
            try:
                ...
                self.logger.warning(
                    "Strategy failed", strategy=strategy.__name__, error=str(e)
                )
                continue

        # If no strategy worked, mark as unresolvable
        self.stats["unresolvable_cases"] += 1
        return {"resolved": [], "ambiguous": [group]}
    ...
    def _resolve_by_confidence(
        ...
        sorted_matches = sorted(
            group, key=lambda m: m.match_confidence.overall_confidence, reverse=True
        )
        ...
        for match in sorted_matches:
            ...
            if (
                match.image_node_id in used_images
                or match.caption_node_id in used_captions
            ):
                continue
    ...
    def _resolve_by_spatial_preference(
        ...
        def spatial_quality_score(match: SpatialMatch) -> float:
            ...
            if self.config.prefer_aligned_captions:
                ...
                horizontal_overlap = min(
                    match.image_bbox.x1, match.caption_bbox.x1
        ...
        sorted_matches = sorted(group, key=spatial_quality_score, reverse=True)
        ...
        for match in sorted_matches:
            if (
                match.image_node_id not in used_images
                and match.caption_node_id not in used_captions
            ):
                pair = self._create_image_caption_pair(match, graph)
                resolved.append(pair)
                used_images.add(match.image_node_id)
        ...
        remaining_matches = [
            ...
            for m in group
            if (
                m.image_node_id not in used_images
                or m.caption_node_id not in used_captions
            )
        ]
    ...
    def _resolve_by_semantic_strength(
        ...
        def semantic_strength(match: SpatialMatch) -> float:
            ...
            strong_keyword_bonus = sum(
                0.2 for keyword in match.keywords_found if keyword in strong_indicators
            )

            return semantic_confidence + keyword_score + strong_keyword_bonus

        # Sort by semantic strength
        sorted_matches = sorted(group, key=semantic_strength, reverse=True)
        ...
        for match in sorted_matches:
            if (
                match.image_node_id not in used_images
                and match.caption_node_id not in used_captions
                and semantic_strength(match) >= 0.5
    ...
    def _handle_shared_caption_pattern(
        ...
        best_match = max(group, key=lambda m: m.match_confidence.overall_confidence)
        ...
        return {"resolved": [pair], "ambiguous": []}
    ...
    def _calculate_pair_quality_score(self, match: SpatialMatch) -> float:
        ...
        quality_score = (
            spatial_quality * 0.4 + confidence_quality * 0.4 + keyword_quality * 0.2
        )
    ...
    def _update_uniqueness_confidence(
        ...
        for case in ambiguous_cases:
            for match in case:
                ...
                caption_competition[caption_id] = (
                    caption_competition.get(caption_id, 0) + 1
                )
    ...
    def _create_final_result(
        ...
        result = MatchingResult(
            successful_pairs=resolved_pairs, ambiguous_matches=ambiguous_cases
        )
