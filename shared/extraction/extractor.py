...
class ContributorExtractor:
        ...
        try:
            ...
            if SPACY_AVAILABLE:
                try:
                    ...
                    self.logger.warning(
                        "spaCy model not found, falling back to basic model"
                    )
                    try:
            ...
            if TRANSFORMERS_AVAILABLE and self.config.use_transformers:
                try:
                    ...
                    self.logger.info(
                        "Loaded Transformer model", model=self.config.transformer_model
                    )
                except Exception as e:
                    self.logger.warning(
                        "Failed to load Transformer model", error=str(e)
                    )
                    self.transformer_pipeline = None
    ...
    def extract_contributors(self, text: str) -> ExtractionResult:
        ...
        try:
            ...
            all_matches = self._combine_matches(
                pattern_matches, ner_matches + edge_case_matches
            )
            ...
            contributors = self.edge_case_handler.optimize_contributor_list(
                contributors
            )
            ...
            self.logger.error(
                "Error in contributor extraction", error=str(e), exc_info=True
            )
            ...
            for ent in doc.ents:
                if ent.label_ == "PERSON":
                    ...
                    match = ContributorMatch(
                        ...
                        context_before=text[
                            max(0, ent.start_char - 50) : ent.start_char
                        ],
                        context_after=text[ent.end_char : ent.end_char + 50],
                        extraction_method="spacy_ner",
                        extraction_confidence=float(ent._.confidence)
            ...
            for result in ner_results:
                if result["entity_group"] == "PER":  # Person entity
                    ...
                    match = ContributorMatch(
                        ...
                        context_before=text[
                            max(0, result["start"] - 50) : result["start"]
                        ],
    ...
    def _matches_overlap(
        self, match1: ContributorMatch, match2: ContributorMatch
    ) -> bool:
        ...
        min_length = min(
            match1.end_pos - match1.start_pos, match2.end_pos - match2.start_pos
        )
    ...
    def _enhance_role_classification(
        self, matches: List[ContributorMatch], text: str
    ) -> List[ContributorMatch]:
    ...
    def _filter_and_validate_contributors(
        ...
        try:
            ...
            for contributor in contributors:
                ...
                if (
                    contributor.extraction_confidence
                    >= self.config.min_extraction_confidence
                    and contributor.role_confidence >= self.config.min_role_confidence
                    and contributor.name_confidence >= self.config.min_name_confidence
                ):
                    # Check completeness requirement
                    if (
                        not self.config.require_complete_names
                        or contributor.name.is_complete
                    ):
    ...
    def _are_similar_contributors(
        self, contrib1: ExtractedContributor, contrib2: ExtractedContributor
    ) -> bool:
