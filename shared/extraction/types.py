...
class ExtractionError(Exception):
    ...
    def __init__(
        self, message: str, text: Optional[str] = None, pattern: Optional[str] = None
    ):
        self.text = text
...
class ExtractedContributor:
    ...
    def is_high_quality(self) -> bool:
        ...
        return (
            ...
            and self.name.is_complete
            and self.role != ContributorRole.UNKNOWN
        )
...
class ExtractionResult:
    ...
    def photographer_count(self) -> int:
        ...
        return len(
            [c for c in self.contributors if c.role == ContributorRole.PHOTOGRAPHER]
        )
    ...
    def average_confidence(self) -> float:
        ...
        if not self.contributors:
            return 0.0
        return sum(c.overall_confidence for c in self.contributors) / len(
            self.contributors
        )

    def get_contributors_by_role(
        self, role: ContributorRole
...
class ExtractionMetrics:
    ...
    def calculate_performance_score(self) -> float:
        ...
        return self.name_extraction_rate * 0.5 + self.role_classification_accuracy * 0.5

    @property
    def meets_targets(self) -> bool:
        """Check if extraction meets target performance."""
        return (
            self.name_extraction_rate >= 0.99
            and self.role_classification_accuracy >= 0.995
        )
