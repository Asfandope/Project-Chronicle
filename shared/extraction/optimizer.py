...
class PerformanceOptimizer:
    ...
    def optimize_extraction_result(
        self, result: ExtractionResult, original_text: str, config: ExtractionConfig
    ) -> ExtractionResult:
        """
    ...
    def _apply_ensemble_scoring(
        self, contributors: List[ExtractedContributor], original_text: str
        ...
        for contributor in contributors:
            ...
            ensemble_score = sum(
                ensemble_factors[factor] * weight for factor, weight in weights.items()
            )

            # Update confidence scores based on ensemble
            boost_factor = min(1.2, ensemble_score / contributor.overall_confidence)

            contributor.extraction_confidence = min(
                1.0, contributor.extraction_confidence * boost_factor
            )
            contributor.role_confidence = min(
                1.0, contributor.role_confidence * boost_factor
            )
            contributor.name_confidence = min(
                1.0, contributor.name_confidence * boost_factor
            )
    ...
    def _apply_confidence_boosting(
        ...
        for contributor in contributors:
            ...
            if confidence_boost > 0:
                contributor.extraction_confidence = min(
                    1.0, contributor.extraction_confidence + confidence_boost
                )
                contributor.role_confidence = min(
                    1.0, contributor.role_confidence + confidence_boost * 0.8
                )
    ...
    def _apply_context_expansion(
        ...
        for contributor in contributors:
            ...
            role_boost = self._analyze_expanded_context(
                expanded_context, contributor.role
            )

            if role_boost > 0:
                contributor.role_confidence = min(
                    1.0, contributor.role_confidence + role_boost
                )
    ...
    def _apply_adaptive_thresholds(
        self, contributors: List[ExtractedContributor], config: ExtractionConfig
    ) -> List[ExtractedContributor]:
        ...
        avg_extraction_conf = sum(c.extraction_confidence for c in contributors) / len(
            contributors
        )
        ...
        if avg_extraction_conf > 0.9:
            adjusted_extraction_threshold = max(
                0.6, config.min_extraction_confidence - 0.1
            )
        ...
        for contributor in contributors:
            if (
                contributor.extraction_confidence >= adjusted_extraction_threshold
                and contributor.role_confidence >= adjusted_role_threshold
                and contributor.name_confidence >= adjusted_name_threshold
            ):
                optimized.append(contributor)
    ...
    def _calculate_context_score(
        self, contributor: ExtractedContributor, original_text: str
    ) -> float:
    ...
    def _analyze_expanded_context(
        self, context: str, current_role: ContributorRole
    ) -> float:
    ...
    def _assess_optimized_quality(
        ...
        avg_confidence = sum(c.overall_confidence for c in contributors) / len(
            contributors
        )
        high_quality_count = sum(1 for c in contributors if c.is_high_quality)
        quality_ratio = high_quality_count / len(contributors)

        complete_names = sum(1 for c in contributors if c.name.is_complete) / len(
            contributors
        )

        # Higher standards for optimized results
        if avg_confidence >= 0.95 and quality_ratio >= 0.9 and complete_names >= 0.85:
            return "high"
        elif avg_confidence >= 0.85 and quality_ratio >= 0.75 and complete_names >= 0.7:
            return "medium"
        else:
            return "low"

    def _track_performance(
        self, original_result: ExtractionResult, optimized_result: ExtractionResult
    ) -> None:
        ...
        performance_data = {
            ...
            "improvement": len(optimized_result.contributors)
            - len(original_result.contributors),
        }
    ...
    def get_performance_metrics(self) -> Dict[str, any]:
        ...
        quality_improvements = sum(
            1 for d in recent_data if d["optimized_quality"] != d["original_quality"]
        )
    ...
    def calibrate_for_target_performance(
        ...
        extracted_names = sum(
            r.get("correctly_extracted_names", 0) for r in test_results
        )

        total_roles = sum(r.get("total_ground_truth_roles", 0) for r in test_results)
        correct_roles = sum(
            r.get("correctly_classified_roles", 0) for r in test_results
        )

        name_extraction_rate = extracted_names / total_names if total_names > 0 else 0.0
        role_classification_rate = (
            correct_roles / total_roles if total_roles > 0 else 0.0
        )
