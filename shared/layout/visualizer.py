...
class LayoutVisualizer:
    ...
    def create_annotated_pdf(
        ...
        try:
            ...
            try:
                ...
                for page_num in range(start_page, end_page + 1):
                    try:
                        ...
                        self.logger.error(
                            "Error annotating page", page_num=page_num, error=str(e)
                        )
                        continue
            ...
            self.logger.error(
                "Unexpected error creating annotated PDF", error=str(e), exc_info=True
            )
    ...
    def _annotate_page(self, page: fitz.Page, page_layout: PageLayout):
        ...
        try:
            ...
            self.logger.warning(
                "Error annotating page", page_num=page_layout.page_num, error=str(e)
            )
    ...
    def _add_page_title_annotation(self, page: fitz.Page, page_layout: PageLayout):
        ...
        try:
            ...
            # Add text
            page.insert_text(
                text_rect.tl + (2, 12), summary_text, fontsize=10, color=(0, 0, 0)
            )
    ...
    def _annotate_text_block(self, page: fitz.Page, block: TextBlock, block_index: int):
        ...
        try:
            ...
            # Add label text
            page.insert_text(
                label_pos, label_text, fontsize=self.config.text_size, color=(0, 0, 0)
            )

            # Add text content preview if enabled and space allows
            if self.config.show_text and rect.height > 30 and rect.width > 100:
                self._add_text_preview(page, block, rect)

        except Exception as e:
            self.logger.warning(
                "Error annotating text block", block_index=block_index, error=str(e)
            )
            ...
            if available_width > 50 and available_height > 15:
                ...
                page.insert_textbox(
                    fitz.Rect(
                        text_pos.x,
                        text_pos.y,
                        text_pos.x + available_width,
                        text_pos.y + available_height,
                    ),
                    preview_text,
    ...
    def _add_legend(self, page: fitz.Page, page_layout: PageLayout):
        ...
        try:
            ...
            # Add legend title
            page.insert_text(
                legend_rect.tl + (5, 15), "Block Types", fontsize=10, color=(0, 0, 0)
            )
            ...
            for i, block_type in enumerate(block_types):
                ...
                color_rect = fitz.Rect(
                    item_pos.x, item_pos.y - 8, item_pos.x + 12, item_pos.y + 4
                )
    ...
    def create_layout_images(
        ...
        try:
            ...
            try:
                ...
                for page_num in range(start_page, end_page + 1):
                    try:
                        ...
                        self.logger.error(
                            "Error creating layout image for page",
                            page_num=page_num,
                            error=str(e),
                        )
                        continue
                ...
                self.logger.info(
                    "Layout images created", images_created=len(created_images)
                )
            ...
            self.logger.error(
                "Error creating layout images", error=str(e), exc_info=True
            )
            raise LayoutError(f"Layout image creation failed: {str(e)}")

    def _annotate_image(
        self, image: Image.Image, page_layout: PageLayout, matrix: fitz.Matrix
    ) -> Image.Image:
            ...
            for block in page_layout.text_blocks:
                ...
                if self.config.show_bounding_boxes:
                    draw.rectangle(
                        ...
                        outline=color_rgb,
                        fill=color_rgba,
                        width=int(self.config.line_width),
                    )
    ...
    def create_layout_summary_image(
        self, layout_result: LayoutResult, output_path: Path
    ) -> Path:
        """
        ...
        try:
            # Create summary data
            self._prepare_summary_data(layout_result)
    ...
    def _prepare_summary_data(self, layout_result: LayoutResult) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "avg_blocks_per_page": layout_result.total_blocks
                / max(layout_result.page_count, 1),
            }
        except Exception as e:
    ...
    def export_layout_data(
        self, layout_result: LayoutResult, output_path: Path, format: str = "json"
    ):
        """
        ...
        try:
            ...
            self.logger.error(
                "Error exporting layout data", format=format, error=str(e)
            )
