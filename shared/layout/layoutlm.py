...
class LayoutLMClassifier:
    ...
    def __init__(
        ...
        self.logger = logger.bind(
            component="LayoutLMClassifier", model=model_name, device=device
        )
        ...
        if self.use_brand_models:
            try:
                self.brand_manager = BrandModelManager(
                    base_model_name=model_name, device=device
                )
                self.logger.info(
                    "Brand model manager initialized",
                    available_brands=self.brand_manager.get_available_brands(),
                )
            except Exception as e:
                self.logger.warning(
                    "Failed to initialize brand model manager", error=str(e)
                )
    ...
    def load_model(self, brand: Optional[str] = None):
        ...
        try:
            ...
            self.logger.error(
                "Error loading LayoutLM model", error=str(e), exc_info=True
            )
    ...
    def _load_base_model(self):
        ...
        self.processor = LayoutLMv3Processor.from_pretrained(
            self.model_name, apply_ocr=False  # We handle OCR separately
        )
        ...
        self.model = LayoutLMv3ForTokenClassification.from_pretrained(
            self.model_name, num_labels=len(self.block_type_mapping)
        )
    ...
    def _load_brand_model(self, brand: str):
        ...
        try:
            ...
            (
                ...
                tokenizer,
                model_info,
            ) = self.brand_manager.get_model_for_brand(brand)
            ...
            self.logger.info(
                "Brand-specific model loaded successfully",
                brand=brand,
                is_fine_tuned=model_info.is_fine_tuned,
                accuracy=model_info.accuracy,
            )
            ...
            self.logger.warning(
                "Failed to load brand model, falling back to base",
                brand=brand,
                error=str(e),
            )
    ...
    def classify_blocks(
        ...
        try:
            ...
            self.logger.debug(
                "Classifying blocks with LayoutLM", block_count=len(text_blocks)
            )
            ...
            self.logger.error(
                "Error in LayoutLM classification", error=str(e), exc_info=True
            )
    ...
    def _create_synthetic_page_image(
        self, text_blocks: List[TextBlock], page_layout: Optional[PageLayout]
    ) -> Image.Image:
        ...
        for block in text_blocks:
            ...
            if block_predictions:
                ...
                matching_confidences = [
                    conf
                    for pred, conf in zip(block_predictions, block_confidences)
                    if pred == block_type
                ]
                avg_confidence = (
                    np.mean(matching_confidences) if matching_confidences else 0.0
                )
    ...
    def _apply_brand_adjustments(self, blocks: List[TextBlock]) -> List[TextBlock]:
        ...
        try:
            ...
            self.logger.debug(
                "Applied brand adjustments", brand=self.brand_config.get("name")
            )
    ...
    def _apply_confidence_thresholding(
        ...
        for block in blocks:
            if block.confidence < self.confidence_threshold:
                ...
                fallback_type, fallback_confidence = self._apply_fallback_heuristics(
                    block
                )
    ...
    def get_classification_metrics(self, blocks: List[TextBlock]) -> Dict[str, Any]:
        ...
        metrics["accuracy_estimate"] = min(
            0.995, 0.85 + 0.15 * metrics["avg_confidence"]
        )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self.use_brand_models and self.brand_manager:
            ...
            base_info[
                "model_comparison"
            ] = self.brand_manager.get_model_performance_comparison()
    ...
    def create_brand_config(
        brand_name: str, config_path: Optional[Path] = None
    ) -> Dict[str, Any]:
        ...
        if config_path and config_path.exists():
            try:
                ...
                logger.warning(
                    "Error loading brand config", path=str(config_path), error=str(e)
                )
        ...
        brand_configs = {
            ...
            "generic": {"name": "Generic Publication", "confidence_adjustments": {}},
        }
