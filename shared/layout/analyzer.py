...
class LayoutAnalyzer:
    ...
    def analyze_pdf(
        ...
        try:
            ...
            if not pdf_path.exists():
                raise LayoutError(
                    f"PDF file does not exist: {pdf_path}", pdf_path=pdf_path
                )
            ...
            self.logger.error(
                "Unexpected error in PDF layout analysis",
                pdf_path=str(pdf_path),
                error=str(e),
                exc_info=True,
            )
            raise LayoutError(
                f"PDF layout analysis failed: {str(e)}", pdf_path=pdf_path
            )
    ...
    def analyze_page(self, doc: fitz.Document, page_index: int) -> PageLayout:
        ...
        try:
            ...
            if self.config.enable_reading_order:
                classified_blocks = self._determine_reading_order(
                    classified_blocks, page_rect
                )
            ...
            self.logger.error(
                "Error analyzing page layout", page_num=page_num, error=str(e)
            )
            raise LayoutError(
                f"Page {page_num} layout analysis failed: {str(e)}", page_num
            )
    ...
    def _extract_text_blocks(self, page: fitz.Page, page_num: int) -> List[TextBlock]:
        ...
        try:
            ...
            self.logger.error(
                "Error extracting text blocks", page_num=page_num, error=str(e)
            )
            return []
    ...
    def _analyze_block_fonts(self, block: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "is_bold": is_bold,
                "is_italic": is_italic,
                "size_variation": max(font_sizes) - min(font_sizes)
                if len(font_sizes) > 1
                else 0,
    ...
    def _should_merge_blocks(self, block1: TextBlock, block2: TextBlock) -> bool:
        ...
        try:
            ...
            if distance <= self.config.merge_distance_threshold:
                ...
                h_overlap = min(block1.bbox.y1, block2.bbox.y1) - max(
                ...
                v_overlap = min(block1.bbox.x1, block2.bbox.x1) - max(
    ...
    def _determine_reading_order(
        self, text_blocks: List[TextBlock], page_rect: fitz.Rect
    ) -> List[TextBlock]:
        ...
        try:
            ...
            summary = {
                ...
                "layout_features": {
                    ...
                    "has_page_numbers": blocks_by_type.get(BlockType.PAGE_NUMBER, 0)
                    > 0,
                ...
                "quality_indicators": {
                    ...
                    "classification_rate": (
                        total_blocks - blocks_by_type.get(BlockType.UNKNOWN, 0)
                    )
                    / max(total_blocks, 1),
                    "processing_speed": total_blocks
                    / max(result.total_processing_time, 1),  # blocks per second
                },
            }
