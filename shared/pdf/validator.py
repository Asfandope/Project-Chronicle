...
class PDFValidator:
    ...
    def __init__(
        self, max_file_size_mb: int = 500, min_pages: int = 1, max_pages: int = 1000
    ):
        """
    ...
    def validate(self, pdf_path: Path, quick_check: bool = False) -> PDFInfo:
        ...
        try:
            ...
            try:
                ...
                if page_count < self.min_pages:
                    validation_errors.append(
                        f"Too few pages: {page_count} < {self.min_pages}"
                    )

                if page_count > self.max_pages:
                    validation_errors.append(
                        f"Too many pages: {page_count} > {self.max_pages}"
                    )
            ...
            self.logger.error(
                "Unexpected error during PDF validation",
                pdf_path=str(pdf_path),
                error=str(e),
                exc_info=True,
            )
    ...
    def _perform_detailed_validation(self, doc: fitz.Document) -> List[str]:
        ...
        try:
            ...
            if corrupted_pages:
                if (
                    len(corrupted_pages) > doc.page_count * 0.5
                ):  # More than 50% corrupted
                    errors.append(
                        f"More than half of pages are corrupted: {len(corrupted_pages)}/{doc.page_count}"
                    )
                else:
                    errors.append(
                        f"Corrupted pages detected: {corrupted_pages[:10]}{'...' if len(corrupted_pages) > 10 else ''}"
                    )
    ...
    def _create_basic_page_info(
        self, doc: fitz.Document, quick_check: bool
    ) -> List[Any]:
        ...
        for page_num in range(doc.page_count):
            try:
                ...
                self.logger.warning(
                    "Error processing page for validation",
                    page_num=page_num + 1,
                    error=str(e),
                )
    ...
    def batch_validate(
        self, pdf_paths: List[Path], quick_check: bool = True
    ) -> Dict[Path, bool]:
        """
