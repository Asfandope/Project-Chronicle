...
class PageSplitter:
    ...
    def __init__(
        self, output_dir: Optional[Path] = None, preserve_metadata: bool = True
    ):
        """
        ...
        try:
            ...
            try:
                ...
                if start_page > end_page:
                    raise PageSplitError(
                        f"Invalid page range: {start_page} > {end_page}", pdf_path
                    )
                ...
                for page_num in range(start_page - 1, end_page):  # Convert to 0-indexed
                    try:
                        ...
                        self.logger.error(
                            "Error splitting page", page_num=page_num + 1, error=str(e)
                        )
            ...
            self.logger.error(
                "Unexpected error during page splitting",
                pdf_path=str(pdf_path),
                error=str(e),
                exc_info=True,
            )
        ...
        try:
            ...
            if self.preserve_metadata:
                ...
                if original_metadata:
                    ...
                    new_metadata[
                        "title"
                    ] = f"{original_metadata.get('title', '')} - Page {page_num + 1}"
    ...
    def split_to_memory(
        ...
        try:
            self.logger.info(
                "Starting in-memory PDF page splitting",
                pdf_path=str(pdf_path),
                page_range=page_range,
            )
            ...
            try:
                ...
                for page_num in range(start_page - 1, end_page):  # Convert to 0-indexed
                    try:
                        ...
                        self.logger.error(
                            "Error processing page in memory",
                            page_num=page_num + 1,
                            error=str(e),
                        )
            ...
            self.logger.error(
                "Unexpected error during in-memory page splitting",
                pdf_path=str(pdf_path),
                error=str(e),
                exc_info=True,
            )
    ...
    def extract_page_ranges(
        ...
        try:
                        ...
                        if self.preserve_metadata:
                            ...
                            if original_metadata:
                                ...
                                if start_page == end_page:
                                    new_metadata[
                                        "title"
                                    ] = f"{original_metadata.get('title', '')} - Page {start_page}"
                                else:
                                    new_metadata[
                                        "title"
                                    ] = f"{original_metadata.get('title', '')} - Pages {start_page}-{end_page}"
                                new_doc.set_metadata(new_metadata)
            ...
            self.logger.error(
                "Unexpected error during page range extraction",
                pdf_path=str(pdf_path),
                error=str(e),
                exc_info=True,
            )
            raise PageSplitError(
                f"Unexpected range extraction error: {str(e)}", pdf_path
            )
    ...
    def get_page_info(self, pdf_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise PageSplitError(
                f"Cannot read PDF page information: {str(e)}", pdf_path
            )
