...
from .types import BoundingBox, PDFProcessingError, TextBlock
...
class TextBlockExtractor:
    ...
    def extract_from_page(
        self, pdf_path: Path, page_num: int, use_ocr_fallback: bool = True
    ) -> List[TextBlock]:
        """
        ...
        try:
            self.logger.info(
                "Extracting text blocks from page",
            ...
            try:
                ...
                if (
                    not text_blocks or self._has_insufficient_text(text_blocks)
                ) and use_ocr_fallback:
                    self.logger.info(
                        "No native text found, attempting OCR fallback",
                        page_num=page_num,
                    )
            ...
            raise TextExtractionError(
                f"Unexpected text extraction error: {str(e)}", pdf_path, page_num
            )
    ...
    def extract_from_pdf(
        ...
        try:
            ...
            raise TextExtractionError(
                f"Unexpected PDF text extraction error: {str(e)}", pdf_path
            )
        ...
        try:
            ...
            for block_idx, block in enumerate(blocks.get("blocks", [])):
                ...
                if (
                    block_text.strip()
                    and block_bbox
                    and len(block_text.strip()) >= self.min_text_length
                ):
            ...
            self.logger.warning(
                "Error in native text extraction", page_num=page_num, error=str(e)
            )
            return []
    ...
    def _extract_with_ocr(self, page: fitz.Page, page_num: int) -> List[TextBlock]:
        ...
        try:
            self.logger.info(
                "OCR text extraction not implemented - returning empty blocks",
                page_num=page_num,
            )
            ...
            self.logger.warning(
                "Error in OCR text extraction", page_num=page_num, error=str(e)
            )
            return []
    ...
    def _classify_text_type(
        ...
        try:
            ...
            if any(
                keyword in text_lower for keyword in ["by ", "photo by", "courtesy of"]
            ):
                return "attribution"
    ...
    def _should_merge_blocks(self, block1: TextBlock, block2: TextBlock) -> bool:
        ...
        try:
            ...
            if vertical_distance < self.merge_distance_threshold:
                ...
                horizontal_overlap = min(block1.bbox.x1, block2.bbox.x1) - max(
            ...
            if horizontal_distance < self.merge_distance_threshold:
                ...
                vertical_overlap = min(block1.bbox.y1, block2.bbox.y1) - max(
                    block1.bbox.y0, block2.bbox.y0
                )
    ...
    def get_page_text_summary(self, text_blocks: List[TextBlock]) -> Dict[str, Any]:
        ...
        avg_confidence = sum(block.confidence for block in text_blocks) / len(
            text_blocks
        )
