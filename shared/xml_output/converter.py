...
class ArticleXMLConverter:
    ...
    def __init__(self, config: Optional[XMLConfig] = None):
        ...
        self.validator = (
            SchemaValidator(self.config) if self.config.validate_output else None
        )
    ...
    def convert_article(
        ...
        try:
            ...
            self.logger.info(
                "Starting article XML conversion",
            ...
            self.logger.info(
                "Article XML conversion completed",
            ...
            self.logger.error(
                "Error in article XML conversion", error=str(e), exc_info=True
            )
    ...
    def _create_xml_document(self, article_data: ArticleData) -> etree.Element:
        ...
        if article_data.contributors:
            contributors_elem = self._create_contributors_element(
    ...
    def _create_contributors_element(
        self, contributors_data: List[Dict[str, Any]]
    ) -> etree.Element:
    ...
    def _create_media_element(self, images_data: List[Dict[str, Any]]) -> etree.Element:
        ...
        for image_data in images_data:
            ...
            if "confidence" in image_data:
                ...
                confidence_str = self._format_confidence(
                    image_data["pairing_confidence"]
                )
                image_elem.set("confidence", confidence_str)
                self.stats["confidence_scores_added"] += 1
    ...
    def _dict_to_article_data(self, data_dict: Dict[str, Any]) -> ArticleData:
        ...
        article_data = ArticleData(
            ...
            processing_pipeline_version=data_dict.get(
    ...
    def _update_conversion_stats(
        self, result: ConversionResult, processing_time: float
    ) -> None:
