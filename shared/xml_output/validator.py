...
from .types import ValidationError, ValidationResult, XMLConfig
...
class SchemaValidator:
    ...
    def _load_schema(self) -> None:
        ...
        try:
            ...
            if self.config.schema_location:
                ...
                schema_path = (
                    Path(__file__).parent.parent.parent / "schemas" / "article-v1.0.xsd"
                )
    ...
    def validate_xml_string(self, xml_content: str) -> ValidationResult:
        ...
        try:
            self.logger.debug(
                "Starting XML validation", content_length=len(xml_content)
            )
    ...
    def validate_xml_file(self, xml_path: Path) -> ValidationResult:
        ...
        try:
            ...
            self.logger.error(
                "Error reading XML file", path=str(xml_path), error=str(e)
            )
            result = ValidationResult(is_valid=False)
    ...
    def _create_parse_error_result(
        self, parse_error: etree.XMLSyntaxError, validation_time: float
    ) -> ValidationResult:
    ...
    def _perform_strict_validation(
        self, xml_doc: etree.Element, result: ValidationResult
    ) -> None:
        ...
        for elem in xml_doc.iter():
            if "bbox" in elem.attrib:
                ...
                try:
                    ...
                    if len(coords) != 4:
                        result.add_error(
                            f"Invalid bounding box format in {elem.tag}: {bbox_str}"
                        )
                    elif coords[2] <= coords[0] or coords[3] <= coords[1]:
                        result.add_warning(
                            f"Invalid bounding box coordinates in {elem.tag}: {bbox_str}"
                        )
