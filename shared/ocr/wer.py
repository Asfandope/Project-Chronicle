...
from .types import DocumentType, OCRError, OCRResult, PageOCRResult
...
class WERCalculator:
    ...
    def calculate_wer(
        ...
        try:
            ...
            char_metrics = self._calculate_character_metrics(
            ...
            problematic_words = self._find_problematic_words(
                ref_normalized, hyp_normalized
            )
    ...
    def calculate_ocr_result_wer(
        self, ocr_result: OCRResult, reference_texts: Dict[int, str]
    ) -> WERMetrics:
        """
        ...
        try:
            ...
            return self.calculate_wer(
                combined_ref, combined_hyp, ocr_result.document_type
            )
    ...
    def calculate_page_wer(
        self, page_result: PageOCRResult, reference_text: str
    ) -> WERMetrics:
        """
        ...
        """
        return self.calculate_wer(
            reference_text, page_result.full_text, page_result.document_type
        )
    ...
    def _calculate_word_metrics(
        ...
        try:
            ...
            return {
                ...
                "correct_words": len(reference.split())
    ...
    def _fallback_word_calculation(
        self, reference: str, hypothesis: str
    ) -> Dict[str, Any]:
    ...
    def _calculate_character_metrics(
        self, reference: str, hypothesis: str
    ) -> Dict[str, Any]:
                ...
                if tag == "replace":
                    ...
                    for k in range(max(i2 - i1, j2 - j1)):
                        if k < i2 - i1 and k < j2 - j1:
                            operations.append(
                                ("substitute", seq1[i1 + k], seq2[j1 + k])
                            )
                        elif k < i2 - i1:
    ...
    def _analyze_error_patterns(
        ...
        try:
            ...
            ocr_patterns = {
                "rn->m": 0,
                "m->rn": 0,  # rn/m confusion
                "cl->d": 0,
                "d->cl": 0,  # cl/d confusion
                "vv->w": 0,
                "w->vv": 0,  # vv/w confusion
                "l->I": 0,
                "I->l": 0,  # l/I confusion
                "0->O": 0,
                "O->0": 0,  # 0/O confusion
    ...
    def validate_against_target(
        self, wer_metrics: WERMetrics, document_type: DocumentType
    ) -> bool:
        ...
        try:
            ...
            if wer_metrics.wer > wer_metrics.target_wer:
                assessment["recommendations"].append(
                    "WER exceeds target - consider preprocessing improvements"
                )

            if wer_metrics.cer > 0.05:  # >5% character error rate
                assessment["recommendations"].append(
                    "High character error rate - check image quality"
                )

            if len(wer_metrics.problematic_words) > 10:
                assessment["recommendations"].append(
                    "Many problematic words - consider vocabulary tuning"
                )

            # Check for specific error patterns
            if any(
                "rn->m" in pattern or "m->rn" in pattern
                for pattern in wer_metrics.error_patterns
            ):
                assessment["recommendations"].append(
                    "rn/m confusion detected - improve image resolution"
                )

            if any(
                "0->O" in pattern or "O->0" in pattern
                for pattern in wer_metrics.error_patterns
            ):
                assessment["recommendations"].append(
                    "Number/letter confusion - consider font-specific training"
                )
    ...
    def compare_multiple_results(
        self, results: List[Tuple[str, WERMetrics]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if comparison["best_wer"] > 0.05:
                comparison["recommendations"].append(
                    "All methods have high WER - consider different preprocessing"
                )

            wer_variance = np.var([metrics.wer for _, metrics in results])
            if wer_variance > 0.01:
                comparison["recommendations"].append(
                    "High variance between methods - ensemble approach may help"
                )
