...
from .types import DocumentType, OCRConfig, OCRError, OCRResult, QualityMetrics
...
class OCRStrategyConfig:
    ...
    def from_yaml(cls, config_path: Path) -> "OCRStrategyConfig":
        ...
        try:
            ...
            logger.error(
                "Error loading OCR strategy config",
...
class OCRStrategy:
    ...
    def _load_brand_configurations(self, configs_dir: Path):
        ...
        try:
            if not configs_dir.exists():
                self.logger.warning(
                    "Brand configs directory not found", dir=str(configs_dir)
                )
                return
            ...
            for config_file in configs_dir.glob("*.yaml"):
                try:
                    ...
                    if "ocr" in config_data:
                        ...
                        self.logger.info("Loaded brand OCR config", brand=brand_name)

                except Exception as e:
                    self.logger.error(
                        "Error loading brand OCR config",
    ...
    def process_pdf(
        ...
        try:
            self.logger.info(
                "Starting OCR strategy processing", pdf_path=str(pdf_path), brand=brand
            )
            ...
            self._get_brand_config(brand)

            # Step 3: Process with OCR engine
            ocr_result = self.engine.process_pdf(
                pdf_path=pdf_path, brand=brand, page_range=page_range
            )
            ...
            if reference_text and self.config.enable_wer_validation:
                wer_metrics = self.wer_calculator.calculate_wer(
                    reference_text, ocr_result.total_text, document_type
                )
            ...
            quality_metrics = self._assess_overall_quality(
                ocr_result,
                wer_metrics,
                confidence_metrics if self.config.enable_confidence_analysis else None,
            )
            ...
            self.logger.info(
                "OCR strategy processing completed",
            ...
            self.logger.error(
                "Unexpected error in OCR strategy",
                pdf_path=str(pdf_path),
                error=str(e),
                exc_info=True,
            )
    ...
    def benchmark_preprocessing(
        ...
        try:
            ...
            for i, prep_config in enumerate(preprocessing_variants):
                try:
                    self.logger.info(
                        f"Testing preprocessing variant {i+1}",
                        variant=i + 1,
                        total=len(preprocessing_variants),
                    )

                    # Temporarily update preprocessor config
                    original_config = (
                        self.preprocessor.get_brand_config(brand)
                        if brand
                    ...
                    wer_metrics = self.wer_calculator.calculate_wer(
                        reference_text, ocr_result.total_text, ocr_result.document_type
                    )
    ...
    def _assess_overall_quality(
        ...
        try:
            ...
            if confidence_metrics:
                ...
                quality_metrics.reliable_char_ratio = (
                    confidence_metrics.reliable_char_ratio
                )
                quality_metrics.uncertain_char_ratio = (
                    confidence_metrics.uncertain_char_ratio
                )

            # Determine quality flags
            quality_metrics.high_confidence_text = quality_metrics.avg_confidence > 0.9
            quality_metrics.requires_review = quality_metrics.avg_confidence < 0.7 or (
    ...
    def _validate_quality_targets(
        self, ocr_result: OCRResult, document_type: DocumentType
    ):
        ...
        try:
            ...
            if (
                hasattr(ocr_result.quality_metrics, "wer")
                and ocr_result.quality_metrics.wer
            ):
        ...
        try:
            ...
            if worst_wer - best_wer > 0.01:  # >1% difference
                recommendations.append(
                    "Preprocessing choice significantly impacts accuracy"
                )
            ...
            self.logger.warning(
                "Error generating preprocessing recommendations", error=str(e)
            )
        ...
        try:
            return {
                ...
                "performance": {
                    ...
                    "avg_time_per_page": ocr_result.total_processing_time
                    / max(ocr_result.page_count, 1),
                    "words_per_second": ocr_result.total_words
                    / max(ocr_result.total_processing_time, 1),
                },
                ...
                "meets_targets": {
                    ...
                    "wer": ocr_result.quality_metrics.meets_wer_target
