...
class DocumentTypeDetector:
    ...
    def detect(
        self, pdf_path: Path, max_pages_to_analyze: int = 5
        ...
        try:
            ...
            try:
                ...
                for page_num in range(pages_to_check):
                    try:
                        ...
                        if self.image_analysis_enabled:
                            image_evidence = self._analyze_image_coverage(
                                page, page_num
                            )
                        ...
                        self.logger.warning(
                            "Error analyzing page for type detection",
                            page_num=page_num,
                            error=str(e),
                        )
                        continue
                ...
                self.logger.info(
                    "Document type detection completed",
            ...
            self.logger.error(
                "Error in document type detection",
                pdf_path=str(pdf_path),
                error=str(e),
                exc_info=True,
            )
    ...
    def _analyze_text_density(self, page: fitz.Page, page_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            text_blocks = [
                block for block in text_dict.get("blocks", []) if "lines" in block
            ]
            ...
            extractability_score = min(
                1.0, (text_length / 100) * 0.1
            )  # Normalized by expected text
            ...
            self.logger.warning(
                "Error in text density analysis", page_num=page_num, error=str(e)
            )
    ...
    def _analyze_fonts(self, page: fitz.Page, page_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            has_standard_fonts = any(
                font in fonts_detected
                for font in ["Times", "Arial", "Helvetica", "Calibri", "Georgia"]
            )
            ...
            self.logger.warning(
                "Error in font analysis", page_num=page_num, error=str(e)
            )
            return {"page_num": page_num, "error": str(e)}
    ...
    def _analyze_image_coverage(self, page: fitz.Page, page_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            image_coverage = (
                min(1.0, total_image_area / page_area) if page_area > 0 else 0
            )
            ...
            self.logger.warning(
                "Error in image coverage analysis", page_num=page_num, error=str(e)
            )
            return {"page_num": page_num, "error": str(e)}
    ...
    def _analyze_layout_structure(
        ...
        try:
            ...
            if text_blocks:
                size_consistency = (
                    1.0 - (np.std(block_sizes) / np.mean(block_sizes))
                    if block_sizes
                    else 0
                )
                layout_organization_score = min(
                    1.0, len(text_blocks) / 10.0 + size_consistency * 0.5
                )
            ...
            self.logger.warning(
                "Error in layout structure analysis", page_num=page_num, error=str(e)
            )
            return {"page_num": page_num, "error": str(e)}
    ...
    def _analyze_metadata(self, doc: fitz.Document) -> Dict[str, Any]:
        ...
        try:
            ...
            has_scanning_indicators = any(
                keyword in creator + producer for keyword in scanning_keywords
            )
            has_born_digital_indicators = any(
                keyword in creator + producer for keyword in born_digital_keywords
            )
    ...
    def _classify_document(
        ...
        try:
            ...
            born_digital_evidence = (
                avg_text_score * 0.3
                + avg_font_score * 0.25  # 30% weight on text extractability
                + avg_layout_score * 0.2  # 25% weight on font analysis
                + (1 - avg_image_score)  # 20% weight on layout structure
                * 0.25  # 25% weight on image analysis (inverted)
            )
    ...
    def _summarize_evidence(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Metadata summary
            metadata = evidence["metadata_analysis"]
            summary["has_metadata_indicators"] = metadata.get(
                "has_born_digital_indicators", False
            ) or metadata.get("has_scanning_indicators", False)
    ...
    def _get_classification_factors(self, evidence: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if text_analyses:
                avg_extractability = np.mean(
                    [a["extractability_score"] for a in text_analyses]
                )
    ...
    def _get_confidence_breakdown(self, evidence: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            text_scores = [
                a["extractability_score"]
                for a in evidence["text_analysis"]
                if "error" not in a
            ]
            if text_scores:
            ...
            font_scores = [
                a["font_analysis_score"]
                for a in evidence["font_analysis"]
                if "error" not in a
            ]
            if font_scores:
            ...
            image_scores = [
                a["scanned_score"]
                for a in evidence["image_analysis"]
                if "error" not in a
            ]
            if image_scores:
            ...
            layout_scores = [
                a["layout_organization_score"]
                for a in evidence["layout_analysis"]
                if "error" not in a
            ]
            if layout_scores:
            ...
            metadata = evidence["metadata_analysis"]
            if metadata.get("has_born_digital_indicators") or metadata.get(
                "has_scanning_indicators"
            ):
