        ...
        contributors=[
            {
                ...
                "role": "author",
                "affiliation": "Medical Technology Institute",
            }
        ],
        text_elements=[
            title_element,
        ...
        "articles": [
            {
                ...
                "contributors": [{"name": "Dr. Sarah Johnson", "role": "author"}],
                "media_elements": [
                    {
                        "type": "image",
                        "bbox": (320, 150, 540, 240),
...
def test_accuracy_calculation():
    ...
    for case_name, extracted_doc in test_cases:
        ...
        # Calculate accuracy
        doc_accuracy = calculator.calculate_document_accuracy(
            ground_truth, extracted_doc
        )

        # Print results
        print(
            f"Overall Weighted Accuracy: {doc_accuracy.document_weighted_accuracy:.2%}"
        )
        print()
        print("Field Accuracies:")
        print(
            f"  Title (30%):       {doc_accuracy.overall_title_accuracy.accuracy:.2%}"
        )
        print(
            f"  Body Text (40%):   {doc_accuracy.overall_body_text_accuracy.accuracy:.2%}"
        )
        print(
            f"  Contributors (20%): {doc_accuracy.overall_contributors_accuracy.accuracy:.2%}"
        )
        print(
            f"  Media Links (10%): {doc_accuracy.overall_media_links_accuracy.accuracy:.2%}"
        )
        ...
        if doc_accuracy.article_accuracies:
            ...
            print(
                f"  Body WER: {body_details.get('word_error_rate', 0):.4f} (threshold: {body_details.get('wer_threshold', 0):.4f})"
            )
            print(
                f"  Meets WER threshold: {'✓' if body_details.get('meets_threshold') else '✗'}"
            )
            ...
            if "match_details" in contrib_details:
                for i, match in enumerate(contrib_details["match_details"]):
                    print(
                        f"  Contributor {i+1}: {'✓' if match['is_correct'] else '✗'} (score: {match['match_score']:.2f})"
                    )
            ...
            if "pair_details" in media_details:
                for i, pair in enumerate(media_details["pair_details"]):
                    print(
                        f"  Media Pair {i+1}: {'✓' if pair['is_correct'] else '✗'} (score: {pair['match_score']:.2f})"
                    )
    ...
    test_titles = [
        (
            "Dr. Johnson's Revolutionary AI Technology",
            "doctor johnsons revolutionary ai technology",
        ),
    ...
    for ref, hyp, expected_wer in test_wer_cases:
        ...
        print(
            f"  WER: {actual_wer:.3f} (expected: {expected_wer:.3f}) {'✓' if abs(actual_wer - expected_wer) < 0.001 else '✗'}"
        )
        print()
